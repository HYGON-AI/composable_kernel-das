...
struct Relu
{
    ...
    template <>
    CK_TILE_HOST_DEVICE void operator()(ck_tile::fp16x2_t& y, const ck_tile::fp16x2_t& x) const
    {
        asm volatile("v_pk_max_f16 %0 %1 0;\n\t" : "=v"(y) : "v"(x));
    }
...
struct Gelu
{
    ...
    template <>
    CK_TILE_HOST_DEVICE void
    operator()<fp32x2_t, fp32x2_t>(fp32x2_t& y, const fp32x2_t& x) const
    {
        y.x = 0.5f * x.x * (1.f + erf(float(0.70710678118f * x.x)));
        y.y = 0.5f * x.y * (1.f + erf(float(0.70710678118f * x.y)));
    }
