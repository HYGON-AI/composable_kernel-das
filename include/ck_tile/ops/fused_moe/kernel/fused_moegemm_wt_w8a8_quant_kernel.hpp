...
struct FusedMoeGemmWWtW8A8QuantKernel
{
...
#undef _TS_
    ...
    CK_TILE_HOST static constexpr auto GridSize(const Hargs& hargs)
    {
        ...
        int max_num_m_block = hargs.topk * hargs.num_tokens;
        int max_num_tokens_padded =
            max_num_m_block + hargs.num_experts * block_m - hargs.topk;
        // printf("xxx max_num_tokens_padded:%d\n", max_num_tokens_padded);
        return Partitioner::GridSize(max_num_tokens_padded, hargs.intermediate_size, max_num_m_block);
    }
        ...
        else
        {
            ...
            const auto g_window = [&]() {
                ...
                const auto g_view_ = make_naive_tensor_view<address_space_enum::global>(
                    ...
                    make_tuple(kargs.hidden_size, number<1>{}),
            ...
            // mmac transpose C layout. M: thread 0/16/32/48, N: thread 0-15,32-47,16-31,48-63
            // each warp mmac has 4x16 x 4(regs) data. Atomic add can deal 4 lines one time.
            // Block_M1/4 = 16/4 =4, the result '4' just means 4(regs).
            ...
            auto o_windows = generate_tuple(
                [&](auto i) {
                    ...
                    //         o_ptr,
                    //         make_tuple(kargs.num_tokens, kargs.topk, kargs.hidden_size),
                    //         make_tuple(kargs.stride_token * kargs.topk, kargs.stride_token, 1),
                    ...
                    //                    make_indexing_transform(kargs.topk, out_topk),
                    //                    make_pass_through_transform(kargs.hidden_size)),
                    //         make_tuple(sequence<0>{}, sequence<1>{}, sequence<2>{}),
                    //         make_tuple(sequence<0>{}, sequence<1>{}, sequence<2>{}));

                    //     auto o_merge_view_ = transform_tensor_view(
                    //         o_scatter_view_,
                    //         make_tuple(make_merge_transform(make_tuple(kargs.num_tokens, 1)),
                    //                    make_pass_through_transform(kargs.hidden_size)),
