    ...
    */
    ...
    int smem_rows = [&](){
        ...
        if(r_for_sub_token > 1)
        {
            ...
            int clz_ = __builtin_clz(r_unroll_); // 0b1:31 0b2:30, 0b3:30, 0b4:29           ///! r_unroll_前导0的个数
            int mask_ = (1 << (31 - clz_)) - 1;                                             ///！ r_unroll_的最高有效位之后全设为1， 即mask_

            
            mask_ = mask_ > 0b111 ? 0b111 : mask_;  //clamp to 8x at most                   ///! mask使得最后r_unroll_大于等于8时，取8的倍数；小于8时，保持不变
            mask_ = ~mask_;

            r_for_sub_token = (r_unroll_ & mask_) * sub_unroll;                             ///！ 最终使得r_for_sub_token 为64的倍数
        }
...
{
    const void* p_topk_ids;     // [token, topk]; IDs outside [0, num_experts) are ignored
    ...
    // Default EX/P0-P1-P23 paths: [0,E) local counts (masked=0), [E,2E) padded
    // exclusive offsets in global expert order. Empty experts are also written.
    void* p_tokens_positions_per_expert;
...
struct MoeSortingKernel
{
    ...
    CK_TILE_HOST static constexpr auto GridSize(const Hargs& h)
    {
        ...
        return dim3(1 + ck_tile::integer_divide_ceil(h.moe_buf_bytes, BlockSize(h).x * 16));    //0:排序， 1~... 负责清buf=0
    }
    ...
    CK_TILE_HOST static constexpr auto MakeKargs(const Hargs& h)
    {
        ...
        k.p_tokens_positions_per_expert     = h.p_tokens_positions_per_expert;
    ...
    template <typename data_t, int wave_size, int block_size = 256>
    __device__ inline void wave_cumsum(data_t& thread_data) const
    {
        ...
        if constexpr(wave_size == 8) {
            ...
            // 1. 
            // __shared__ data_t thread_7_tmp[block_size / warpSize * 4];
            // if (threadIdx.x % 16 == 7) {     //每个warp的第7、23、39、55个线程
            //     thread_7_tmp[threadIdx.x / 16] = thread_data;
            // }
            // __syncthreads();
            // data_t xxx = thread_7_tmp[threadIdx.x / 16];

            // 2.
            data_t xxx = __builtin_bit_cast(data_t,
                __builtin_amdgcn_ds_bpermute(((__lane_id() & 0x30) + 7) << 2,
                                                __builtin_bit_cast(int, thread_data)));
...
#pragma unroll Problem_::InternalLoadUnroll
        ...
        {
            const IndexType eid = topk_id[i];
            if(eid >= 0 && eid < num_experts)
                ++tokens_cnts[calc_index(num_experts + 1, tid + 1, eid)];
        }
        __syncthreads();
...
#pragma unroll Problem_::InternalLoadUnroll
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
            p_sorted_token_ids[rank_post_pad] = MOE_SORTING_MOCK_ID(topk_mdiv.div(i), expert_id);
#else
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
                    p_sorted_token_ids[expert_offset] = MOE_SORTING_MOCK_ID(prefill_token, num_experts);
#else
    ...
    moe_align_block_size_kernel_ex(const IndexType* __restrict__ topk_id,
                                   ...
                                   WeightType* p_sorted_weights,
                                   index_t* p_sorted_expert_ids,
                                   index_t* p_tokens_positions_per_expert,
                                   index_t* p_total_tokens_post_pad,
                                   const index_t num_experts,
    ...
    {
        const index_t tid = static_cast<index_t>(threadIdx.x);
        const index_t wid = __builtin_amdgcn_readfirstlane(
            tid / get_warp_size()); ///! 表示同一个warp中的 wid
                             ///是同一个值。其他的线程会从warp的第一个线程中获取数据;
                             ///主要减少寄存器个数,是的 wid 由 VGPR --> SGPR
        ...
        simple_smem_indexer smem_cumsum{reinterpret_cast<index_t*>(smem) + 0}; ///! 第一个元素
        simple_smem_indexer smem_cumdup{reinterpret_cast<index_t*>(smem) +
                                        smem_cols}; ///! 第一个元素
        simple_smem_indexer smem_tokens{reinterpret_cast<index_t*>(smem) + 2 * smem_cols,
                                        smem_cols};

        index_t* p_pos_per_expert = p_tokens_positions_per_expert + num_experts;  //每个expert的起始位置
        ...
        for(int i = tid; i < (sub_tokens * num_experts); i += block_size)
        {
            ...
            expert_mdiv.divmod(
                i, curr_token_id, curr_expert_id);          ///! 求 curr_token_id = i / expert_num;
                                                            ///curr_expert_id = i % expert_num;
            smem_tokens(curr_token_id, curr_expert_id) = 0; ///! smem token缓冲区清0
        }
        __syncthreads();
        ...
        for(int i_token = 0; i_token < tokens; i_token += sub_tokens)
        {
            ...
            for(int i = tid; i < (sub_tokens * topk);
                i += block_size) ///！ for 循环每次处理完sub_tokens * topk个， 每次block_size=256个
            {
                uint32_t curr_token_id, curr_topk_id;
                topk_mdiv.divmod(i, curr_token_id, curr_topk_id);
                ...
                if(i_t < tokens)
                {
                    int eid = topk_id[i_t * topk +
                                      curr_topk_id]; ///！ topk_id shape  = [tokens, topk], dtype =
                                                     ///int32, in HBM. 'eid' is expert index.
                ...
                __builtin_amdgcn_s_waitcnt(
                    0xc07f); ///!
                             ///确保所有内存操作完成，由于同一个token不可能选出两个相同的expert，所以不存在线程写冲突
            }
...

        {
            ...
            constexpr int lane_group_nm = block_size / lane_group_sz; ///! 256 / 8 = 32

            ///! 计算每个expert被选中几次，加和记录在smem_cumsum中
            for(int i_e = lane_group_id; i_e < num_experts;
                i_e += lane_group_nm) ///! 每次处理32个专家，每8个线程处理一个专家
            {
                index_t local_c[Problem::SubTokenTile]; ///! SubTokenTile = 8/4/2/1，
                                                        ///取决于sub_tokens行数能整除几
                index_t cnt = 0;

                for(int i = 0; i < sub_tokens;
                    i += 8 * Problem::SubTokenTile) ///! 本循环每次处理最多8*8=64个tokens
                {
...
#pragma unroll Problem::SubTokenTile
                ...
                if(lane_group_os == 0) {
                    ...
                    if constexpr(Problem::LocalExpertMasking)
                        p_tokens_positions_per_expert[i_e] = local_expert_mask[i_e] ? cnt : 0;
                    else
                        p_tokens_positions_per_expert[i_e] = cnt;
                }
            }
        }

        ///！ smem_cumdup记录是否屏蔽某个expert, 1使能，0屏蔽
        if constexpr(Problem::LocalExpertMasking)
        {
            smem_cumdup(0) = 0;
            ...
            if(wid == 0) ///! 第一个warp
            {
                ...
                for(; i_e_ < num_experts; i_e_ += get_warp_size())
                {
                    ...
                    int blocks_pers_expert = ///!
                                             ///选择本expert的token，一共需要多少个block，block不是gpu
                                             ///kernel执行的block，指的是分块尺寸
                    ...
                    local_cumsum_ = padded_tokens_per_expert; //本expert需要多少个tokens
                    ...
                    if((i_e_ + lid) < num_experts)
                        smem_cumsum(i_e_ + lid + 1) =
                            local_cumsum_; ///! padding后的，各个expert上包含之前expert的token个数
                    ...
                    if constexpr(Problem::LocalExpertMasking)
                    {
                        ...
                        if((i_e_ + lid) < num_experts)
                            smem_cumdup(i_e_ + lid + 1) =
                                local_masking; ///!
                                               ///经过此赋值后，smem_cumdup里除了0号位置外，每个元素都记录了包含自己expert在内，之前一共使能了多少expert
                    }
        ...
        for(int i_e = tid; i_e < num_experts; i_e += block_size)
        {
            ...
            for(int i = e_start; i < e_end; i += unit_size_mdiv.divisor) ///！ 当前expert有token选择
            {
                p_sorted_expert_ids[unit_size_mdiv.div(i)] =
                    expert_id; ///！ 完成token分组后expert的序号填写
            }
        }
        ...
        for(int i_token = 0; i_token < tokens; i_token += sub_tokens)
        {
            if constexpr(!Problem::SubTokenOneShot)
            {
                // clear every time ///! 原来sub_tokens中存的是对应expert的访问次数，全清
                for(int i = tid; i < (sub_tokens * num_experts); i += block_size)
                {
                    uint32_t curr_token_id, curr_expert_id;
...

            {
                ...
                for(int eid = lane_group_id; eid < num_experts;
                    eid += lane_group_nm) ///! 8个线程一组，处理一个expert
                {
                    if constexpr(Problem::LocalExpertMasking)
                    {
                        if(local_expert_mask[eid] == 0) ///! 此expert在本卡无效
                            continue;
                    }
                    int position = smem_cumsum(eid); ///! 本expert起始token序号

                    for(int i_sub_token = lane_group_os; i_sub_token < sub_tokens;
                        i_sub_token += lane_group_sz)
                    {
                        auto x = smem_tokens(i_sub_token, eid); ///！本token 此expert的选择序号

                        int local_cnt_cache = x != 0 ? 1 : 0;
                        int local_cnt       = local_cnt_cache;
                        wave_cumsum<int, lane_group_sz, block_size>(local_cnt);
                        if(x != 0)
                        {
                            // now x is topk value
#if CK_TILE_REFERENCE_MOE_SORTING_MOCK_ID
                            p_sorted_token_ids[position + local_cnt -
                                               1] = ///! token id用低24位，topk id用高8位
                                MOE_SORTING_MOCK_ID(i_token + i_sub_token, x - 1);
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
                            p_sorted_token_ids[position + local_cnt - 1] = MOE_SORTING_MOCK_ID(i_token + i_sub_token, eid);
#else
                        ...
                        int remote_cnt = __builtin_amdgcn_ds_bpermute(
                            (lane_group_sz * (lane_group_id + 1) - 1) << 2,
                            local_cnt); ///!
                                        ///底层调用DS_BPERMUTE_B32，index必须是字节地址，VGPR采用32bit，故乘4
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
                p_sorted_token_ids[e_start] = MOE_SORTING_MOCK_ID(tokens, num_experts);
#else
    ...
    {
        if(blockIdx.x > 0) ///! 除第1个block外，其余block只负责将moe_buf清0，每个线程负责16个字节=0
        {
            if(kargs.p_moe_buf)
            {
        ...
        return moe_align_block_size_kernel_ex(
            ...
            static_cast<IndexType*>(kargs.p_tokens_positions_per_expert),
...
struct MoeSortingMultiPhaseKernel_P1
{
    ...
    CK_TILE_HOST static constexpr auto MakeKargs(const Hargs& h)
    {
        ...
        k.p_tokens_positions_per_expert = reinterpret_cast<index_t*>(h.p_tokens_positions_per_expert);
        k.mesh_stride = impl::moe_sorting_mp_mesh_stride(h.tokens);
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        if constexpr(Problem::LocalExpertMasking)
        {
            ...
            if(mask == 0)
            {
                if(threadIdx.x == 0)
                {
                    p_expert_cumsum[eid] = 0;
                    kargs.p_tokens_positions_per_expert[eid] = 0;
                }
                return; // skip
            }
        }
        ...
        if(threadIdx.x == 0)
        {
            ...
            kargs.p_tokens_positions_per_expert[eid] = c;
        }
    }
};
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
                p_sorted_token_ids[e_start + position] = MOE_SORTING_MOCK_ID(i_token, eid);
#else
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
            p_sorted_token_ids[i] = MOE_SORTING_MOCK_ID(kargs.tokens, kargs.num_experts);
#else
...
struct MoeSortingMultiPhaseKernel_P23
{
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
...

        {
            ...
            const WeightType* p_weights     = static_cast<const WeightType*>(kargs.p_weights);
            WeightType* p_sorted_weights    = reinterpret_cast<WeightType*>(kargs.p_sorted_weights);
            index_t* p_pos_per_expert = kargs.p_tokens_positions_per_expert + kargs.num_experts;
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
                            p_sorted_token_ids[e_start + position_0] = MOE_SORTING_MOCK_ID(i_token, eid);
#else
...
#elif CK_TILE_REFERENCE_MOE_SORTING_MOCK_EXPERT_ID
                            p_sorted_token_ids[e_start + position_1] = MOE_SORTING_MOCK_ID(i_token, eid);
#else
