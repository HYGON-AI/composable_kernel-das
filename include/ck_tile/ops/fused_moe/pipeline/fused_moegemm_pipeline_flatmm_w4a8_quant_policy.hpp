...
struct FusedMoeGemmPipelineFlatmmW4A8QuantPolicy
{
    ...
    {
        // constexpr auto a_sld_desc = MakeLdsLoadDesc_A<Problem>();    // load时，将store的size除以 (block_k0 / quant_block_size_k)
        ...
        return a_sst_desc.get_element_space_size();
    }
    ...
    CK_TILE_HOST_DEVICE static constexpr ck_tile::index_t GetSmemSize_Bridge()
    {
        ...
        constexpr index_t Block_N_   = Problem::BlockShape::Block_N0;
        constexpr index_t bridge_smem_size = GetSmemSize_A<Problem>() / Block_K_ * Block_N_;
        return bridge_smem_size;
    }
    ...
    {
        constexpr index_t K_vec = Alignment;            //4
        constexpr index_t K_rem = KPerBlock / K_vec;    //64/4=16
        ...
        {
            constexpr index_t K_lan = K_rem;                    //16
            constexpr index_t M_lan = get_warp_size() / K_lan;  //4
            constexpr index_t M_wav = NumWarps;
            static_assert(MPerBlock % (M_lan * M_wav) == 0,
                          "this tile size is too small please check");
            constexpr index_t M_rep = MPerBlock / (M_lan * M_wav);
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_w4a8_G()
    {
        constexpr index_t Block_N  = Problem::BlockShape::Block_N0;
        constexpr index_t Block_K  = Problem::BlockShape::Block_K0 / Problem::PackFactor;  // 64/2=32 bytes
        ...
        constexpr index_t KVecotrLength = GetAlignment_G<Problem>();    // 8 bytes, that leads to k elements number in gemm0 = 16. and this is needed by warp_gemm0
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_w4a8_G_SC()
    {
        constexpr index_t Block_N  = Problem::BlockShape::Block_N0;
        constexpr index_t Block_K  = Problem::BlockShape::Block_K0 / Problem::QuantBlockSizeK;  // 64/64=1
        constexpr index_t NumWarps = Problem::BlockShape::NumWarps;
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_w4a8_D()
    {
        constexpr index_t Block_N_  = Problem::BlockShape::Block_N1;
        constexpr index_t Block_K_  = Problem::BlockShape::Block_K1 / Problem::PackFactor;  // 64/2=32 bytes
        constexpr index_t NumWarps_ = Problem::BlockShape::NumWarps;

        constexpr index_t KVecotrLength = GetAlignment_D<Problem>();        // == 8, that leads to k elements number in gemm1 = 16. and this is needed by warp_gemm1
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsStoreDesc_A()
    {
        ...
        constexpr index_t KVector = GetAlignment_A_input<Problem>();          // block_k0 / 16 
        // TODO: support pad mode
        // constexpr index_t KPad    = KPack;                       // pad between warps

        static_assert(Block_K % KVector == 0);
        constexpr index_t LanesPerK = Block_K / KVector;            // 64 / 4 = 16
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsLoadDesc_A()
    {
        ...
        static_assert(Block_K % KVector == 0);
        constexpr index_t LanesPerK = Block_K / KVector;                // 64 / 8 = 8
        if constexpr(LanesPerK >= warpSize)
        {
        ...
        else
        {
            ...
            static_assert(warpSize % LanesPerK == 0);
            constexpr index_t LaneGroups = warpSize / LanesPerK;    // 8
            constexpr index_t NumIssues =
                Block_M / (LaneGroups * Problem::BlockShape::WarpPerBlock_M0);

            constexpr auto lds_block_desc_0 = make_naive_tensor_descriptor(
                make_tuple(number<NumIssues>{},                        // m0 = 2
                           number<LanesPerK>{},                        // k0 = 8
                           number<LaneGroups>{},                       // m1 = 8
                           number<KVector>{}),                         // k1 = 8
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeBridgeTileDistribution()
    {
        ...
        constexpr auto bridge_tile_dstr_enc = tile_distribution_encoding<
            sequence<>,
            tuple<sequence<MRepeat, MWarps, MLane>, sequence<NWarps, kCN0PerLane, NLane, NVector>>,     // <1, 1, 16>, <4, 4, 4, 1>
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetWarpGemm0()
    {
        ...
        if constexpr (Problem::Traits::PermuteEnum == FusedMoeGemmWeightPermuteEnum::no_permute)
        {
            if constexpr(std::is_same_v<typename Problem::GDataType, ck_tile::int8_t> &&
                S_::Warp_M0 == 16 && S_::Warp_N0 == 16 && S_::Warp_K0 == 64)
            {
                return WarpGemmMmacI8I8I32_WT16x16x64_MR1NR1MI1NI1{};

            } else if constexpr(std::is_same_v<typename Problem::GDataType, ck_tile::int8_t> &&
                S_::Warp_M0 == 16 && S_::Warp_N0 == 16 && S_::Warp_K0 == 128)
            {
                return WarpGemmMmacI8I8I32_WT16x16x128_MR1NR1MI1NI1{};
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetWarpGemm1()
    {
        ...
        if constexpr (Problem::Traits::PermuteEnum == FusedMoeGemmWeightPermuteEnum::no_permute)
        {
            if constexpr(std::is_same_v<typename Problem::DDataType, ck_tile::int8_t> &&
                S_::Warp_M1 == 16 && S_::Warp_N1 == 16 && S_::Warp_K1 == 64)
            {
                return WarpGemmMmacI8I8I32_WT16x16x64_MR1NR1MI1NI1_TRANSC{};
    
            } else if constexpr(std::is_same_v<typename Problem::DDataType, ck_tile::int8_t> &&
                S_::Warp_M1 == 16 && S_::Warp_N1 == 32 && S_::Warp_K1 == 64)
            {
                return WarpGemmMmacI8I8I32_WT16x32x64_MR1NR2MI1NI1_TRANSC{};
