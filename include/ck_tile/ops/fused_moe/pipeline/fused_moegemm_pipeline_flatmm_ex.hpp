...
struct FusedMoeGemmPipeline_FlatmmEx
{
    ...
    static constexpr bool kSupportsDeterministicFp64Output = true;
    ...
    template <bool FinalOutput = false,
              bool DebugWriterCount = false,
              typename AWindow,
              typename GWindow,
              typename DWindow,
              typename OWindow,
              typename TopkArrayType,
              typename FinalOWindow = OWindow,
              typename WriterCountWindow = FinalOWindow>
    CK_TILE_DEVICE auto
    operator()(const AWindow& a_window_,
               ...
               FinalOWindow* final_o_window_ = nullptr,
               WriterCountWindow* writer_count_window_ = nullptr
    ...
    )
    {
        ...
        */
        ...
        auto writer_count_windows = [&]() {
            if constexpr(DebugWriterCount)
            {
                return generate_tuple(
                    [&](auto i) {
                        return make_tile_window(
                            (*writer_count_window_)[i],
                            Policy::template MakeGlobalTileDistribution_O<Problem>());
                    },
                    number<atomic_issues>{});
            }
        ...
        // Both manually double-buffered tails below consume two tiles.  Pad-enabled
        // shapes must therefore round the loop count to an even number of tiles;
        // merely rounding to one tile drops the last K0 tile for odd counts (for
        // example 1232 / 32 -> 39, while the old loop executed only 38).
        const index_t num_blocks_k0 = PadHiddenSize
                                          ? ((hidden_size + 2 * BlockShape::Block_K0 - 1) /
                                             (2 * BlockShape::Block_K0)) *
                                                2
                                          : (hidden_size + BlockShape::Block_K0 - 1) /
                                                BlockShape::Block_K0;
        const index_t num_blocks_n1 = PadHiddenSize
                                          ? ((hidden_size + 2 * BlockShape::Block_N1 - 1) /
                                             (2 * BlockShape::Block_N1)) *
                                                2
                                          : (hidden_size + BlockShape::Block_N1 - 1) /
                                                BlockShape::Block_N1;
        ...
        auto atomic_add_o = [&](auto& o_) {
            ...
            static_for<0, atomic_issues, 1>{}([&](auto i) {
                ...
                const auto output_slice =
                    o_.get_y_sliced_thread_data(c_warp_y_index, c_warp_y_lengths);
                constexpr index_t output_size = output_tmp_buf.get_thread_buffer_size();
                static_for<0, output_size, 1>{}([&](auto j) {
                    output_tmp_buf.get_thread_buffer()[j] =
                        output_slice[j] * topk_weights[i];
                });
                if constexpr(FinalOutput)
                {
                    // The owner CTA has already produced chunks 0..4 in FP32.
                    // Add chunk 5 in the same output distribution and perform the
                    // sole FP16 store without writing a sixth FP32 workspace value.
                    auto accumulated_buf = load_tile(o_windows[i]);
                    using final_thread_type = decltype(load_tile(final_o_windows[i]));
                    final_thread_type final_output_buf;
                    static_for<0, output_size, 1>{}([&](auto j) {
                        using FinalDataType = remove_cvref_t<
                            decltype(final_output_buf.get_thread_buffer()[j])>;
                        final_output_buf.get_thread_buffer()[j] = type_convert<FinalDataType>(
                            accumulated_buf.get_thread_buffer()[j] +
                            output_tmp_buf.get_thread_buffer()[j]);
                    });
                    update_tile(final_o_windows[i], final_output_buf);
                    if constexpr(DebugWriterCount)
                    {
                        using count_thread_type = decltype(load_tile(writer_count_windows[i]));
                        count_thread_type count_buf;
                        constexpr index_t count_size = count_buf.get_thread_buffer_size();
                        static_for<0, count_size, 1>{}([&](auto j) {
                            count_buf.get_thread_buffer()[j] = 1;
                        });
                        update_tile(writer_count_windows[i], count_buf);
                        move_tile_window(writer_count_windows[i],
                                         {number<0>{}, number<BlockShape::Block_N1>{}});
                    }
                    move_tile_window(final_o_windows[i],
                                     {number<0>{}, number<BlockShape::Block_N1>{}});
                }
                else
                {
                    update_tile(o_windows[i], output_tmp_buf); // one output buffer issue
                }
        ...
        auto pipeline_bridge = [&]() {
            ...
            else
            {
                ...
                sweep_tile(
                    ...
                    [&](auto idx0, auto idx1) {
                        const YDataType x0 =
                            type_convert<YDataType>(acc_0.at(number<0>{})(idx0));
                        const YDataType x1 =
                            type_convert<YDataType>(acc_0.at(number<0>{})(idx1));
                        AccDataType y0_float;
                        AccDataType y1_float;
                        typename Problem::GateActivation{}(
                            y0_float, type_convert<AccDataType>(x0));
                        typename Problem::GateActivation{}(
                            y1_float, type_convert<AccDataType>(x1));
                        const YDataType y0 = type_convert<YDataType>(y0_float);
                        const YDataType y1 = type_convert<YDataType>(y1_float);
                        acc_0.at(number<0>{})(idx0) = type_convert<AccDataType>(y0);
                        acc_0.at(number<0>{})(idx1) = type_convert<AccDataType>(y1);
                    },
                    sequence<1, 2>{}); // m=1, n=4 continuos element in thread_buffer(VGPR)
                // Match the unfused FP16 contract: GEMM0 output, activation output, and the
                // gate/up product each cross an FP16 rounding boundary before GEMM1.
                auto reduce_acc_0 = tile_elementwise_in(
                    [&](const auto& a_, const auto& b_) {
                        const YDataType activated = type_convert<YDataType>(a_);
                        const YDataType up        = type_convert<YDataType>(b_);
                        const YDataType product   = type_convert<YDataType>(activated * up);
                        return type_convert<AccDataType>(product);
                    },
                    acc_0.at(number<0>{}),
                    acc_0.at(number<1>{}));
        ...
        auto pipeline_gemm1 = [&]() {
            store_tile(d_copy_lds_window, gd[I0]);
            gld_d(gd[I1]);
            move_d();
            block_sync_lds();
            __builtin_amdgcn_sched_barrier(0);
            auto d_tmp = load_tile(d_sld_win);
            gemm_1(acc_1s[I0], y, d_tmp);
            atomic_add_o(acc_1s[I0]);
            block_sync_lds();
            __builtin_amdgcn_sched_barrier(0);
            store_tile(d_copy_lds_window, gd[I1]);
