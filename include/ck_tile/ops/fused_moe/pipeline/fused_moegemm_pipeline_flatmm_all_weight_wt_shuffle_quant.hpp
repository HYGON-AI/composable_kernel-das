...
we need to design the pipeline such that all waves along gemm-N dim (gemm-m only 1 wave)
    ...
    +----+----+----+----+
*/
template <typename Problem_, typename Policy_ = FusedMoeGemmPipelineFlatmmWWtShuffleQuantPolicy>
struct FusedMoeGemmPipeline_Flatmm_Wt_shuffle_quant
{
    ...
    using QT_WarpTile   = ck_tile::sequence<1, get_warp_size()* QT_Vector_N_>;
    ...
    CK_TILE_HOST_DEVICE static constexpr ck_tile::index_t GetSmemSize_quant_out()
    {
        ...
        return Policy::template GetSmemSize_A<Problem>() / Block_K0 * Block_N0;
    }
                              ...
                              index_t loop_index)
{
    ...
    )
    {
        ...
        auto u_view = [&]() {
            if constexpr(IsGateOnly) {
                ...
                const auto u_view_ = make_naive_tensor_view<address_space_enum::global>(
                    u_ptr,
                    make_tuple(hidden_size/number<Block_K0>{}, number<Block_N0 * Block_K0>{}),
                    make_tuple(number<Block_K0 * Block_N0>{}, I1),
                    number<kAlignmentG>{},
                    number<1>{});
                return u_view_;
            }
...
#if 0
        // For test: 1.6 quant A lds to load to register, and then write to global memory to test.
        auto quant_out_lds_view0 =  make_quant_out_tensor_view(smem_quant0);
        auto quant_out_lds_view1 =  make_quant_out_tensor_view(smem_quant1);

        auto quant_out_lds_window0 = make_tile_window(
            quant_out_lds_view0,
            make_tuple(number<Block_M0>{}, number<Block_K0>{}),
            {0, 0},
            Policy::template MakeGlobalTileDistribution_A<Problem>());
        auto quant_out_lds_window1 = make_tile_window(
            quant_out_lds_view1,
            make_tuple(number<Block_M0>{}, number<Block_K0>{}),
            {0, 0},
            Policy::template MakeGlobalTileDistribution_A<Problem>());
        
        // global memory quant out window
        auto quant_out_dram_window = make_tile_window(
            qt_out_window.get_bottom_tensor_view(),
            make_tuple(number<Block_M0>{}, number<Block_K0>{}),
            qt_out_window.get_window_origin());

        using quant_out_thread_type = decltype(load_tile(quant_out_lds_window0));
        statically_indexed_array<quant_out_thread_type, 1> quant_out_r;

        auto write_quant_out = [&](auto& window_) {
            // block_sync_lds();
            load_tile(quant_out_r[I0], window_);
            update_tile(quant_out_dram_window, quant_out_r[I0]);
            move_tile_window(quant_out_dram_window, {number<0>{}, number<Block_K0>{}});
        };
        (void)write_quant_out;
        (void)quant_out_lds_window1;
#endif
        ...
        auto g_copy_dram_window = make_tile_window(
            ...
            make_tuple(I1, number<BlockShape::Block_N0 * BlockShape::Block_K0>{}),
...
#if 0
        // For test: act out lds to load to register, and then write to global memory to test.
        auto act_out_lds_window = make_tile_window(
        ...
        // global memory act out window
        auto act_out_dram_window = make_tile_window(
            act_out_window.get_bottom_tensor_view(),
            make_tuple(number<Block_M0>{}, number<Block_N0>{}),
            act_out_window.get_window_origin());

        using act_out_thread_type = decltype(load_tile(act_out_lds_window));
        statically_indexed_array<act_out_thread_type, 1> act_out_r;

        // due to multiple experts, we just acc this in the hbm, ignore the topk weights
        auto write_act_out = [&]() {
            // block_sync_lds();
            load_tile(act_out_r[I0], act_out_lds_window);
            // update_tile(act_out_dram_window, act_out_r[I0]);
            store_tile(act_out_dram_window, act_out_r[I0]);
        };
        (void)write_act_out;
#endif
        ...
        auto d_copy_dram_window = make_tile_window(
            ...
            make_tuple(I1, number<Block_N1 * Block_K1>{}),
        ...
        auto move_g = [&]() {
            move_tile_window(g_copy_dram_window, {I1, I0});
        };
        ...
        auto move_u = [&]() {
            if constexpr(IsGateOnly) {
                ...
                move_tile_window(u_copy_dram_window, {I1, I0});
            }
        };
        ...
        auto move_d = [&]() {
            move_tile_window(d_copy_dram_window, {I1, I0});
        };
        ...
        auto pipeline_bridge = [&]() {
            ...
            else
            {
                ...
                // if (static_cast<index_t>(blockIdx.x) == 0 && static_cast<index_t>(blockIdx.y) == 0 &&
                // static_cast<index_t>(blockIdx.z) == 0 && static_cast<index_t>(threadIdx.x % 16) < 5)
                // {
                //     printf("bridge_tile_tensor[tid=%d][0~1]: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f\n",
                //         static_cast<int>(threadIdx.x),
                //         bridge_tile_tensor.get_thread_buffer()[0],
                //         bridge_tile_tensor.get_thread_buffer()[1],
                //         bridge_tile_tensor.get_thread_buffer()[2],
                //         bridge_tile_tensor.get_thread_buffer()[3],
                //         bridge_tile_tensor.get_thread_buffer()[4],
                //         bridge_tile_tensor.get_thread_buffer()[5],
                //         bridge_tile_tensor.get_thread_buffer()[6],
                //         bridge_tile_tensor.get_thread_buffer()[7]);
                // }
                ...
                // if (static_cast<index_t>(blockIdx.x) == 0 && static_cast<index_t>(blockIdx.y) == 0 &&
                // static_cast<index_t>(blockIdx.z) == 0 && static_cast<index_t>(threadIdx.x ==0))
                // {
                //     int off = 128;
                //     YDataType *smem_addr = reinterpret_cast<YDataType*>(smem);

                //     printf("quant_in_bdg_win[0]: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f... %.4f, %.4f, %.4f, %.4f\n",
                //         static_cast<float>(smem_addr[0]), static_cast<float>(smem_addr[1]),
                //         static_cast<float>(smem_addr[2]), static_cast<float>(smem_addr[3]),
                //         static_cast<float>(smem_addr[4]), static_cast<float>(smem_addr[5]),
                //         static_cast<float>(smem_addr[6]), static_cast<float>(smem_addr[7]),
                //         static_cast<float>(smem_addr[124]), static_cast<float>(smem_addr[125]),
                //         static_cast<float>(smem_addr[126]), static_cast<float>(smem_addr[127]));
                //     printf("quant_in_bdg_win[1]: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f... %.4f, %.4f, %.4f, %.4f\n",
                //         static_cast<float>(smem_addr[off+0]), static_cast<float>(smem_addr[off+1]),
                //         static_cast<float>(smem_addr[off+2]), static_cast<float>(smem_addr[off+3]),
                //         static_cast<float>(smem_addr[off+4]), static_cast<float>(smem_addr[off+5]),
                //         static_cast<float>(smem_addr[off+6]), static_cast<float>(smem_addr[off+7]),
                //         static_cast<float>(smem_addr[off+124]), static_cast<float>(smem_addr[off+125]),
                //         static_cast<float>(smem_addr[off+126]), static_cast<float>(smem_addr[off+127]));
                        
                //     printf("quant_out_bdg_win[0]: %d, %d, %d, %d, %d, %.d, %d, %d, ... %d, %d, %d, %d\n",
                //         static_cast<int>(smem_quant2[0]), static_cast<int>(smem_quant2[1]),
                //         static_cast<int>(smem_quant2[2]), static_cast<int>(smem_quant2[3]),
                //         static_cast<int>(smem_quant2[4]), static_cast<int>(smem_quant2[5]),
                //         static_cast<int>(smem_quant2[6]), static_cast<int>(smem_quant2[7]),
                //         static_cast<int>(smem_quant2[124]), static_cast<int>(smem_quant2[125]),
                //         static_cast<int>(smem_quant2[126]), static_cast<int>(smem_quant2[127]));
                //     printf("quant_out_bdg_win[1]: %d, %d, %d, %d, %d, %.d, %d, %d, ... %d, %d, %d, %d\n",
                //         static_cast<int>(smem_quant2[off+0]), static_cast<int>(smem_quant2[off+1]),
                //         static_cast<int>(smem_quant2[off+2]), static_cast<int>(smem_quant2[off+3]),
                //         static_cast<int>(smem_quant2[off+4]), static_cast<int>(smem_quant2[off+5]),
                //         static_cast<int>(smem_quant2[off+6]), static_cast<int>(smem_quant2[off+7]),
                //         static_cast<int>(smem_quant2[off+124]), static_cast<int>(smem_quant2[off+125]),
                //         static_cast<int>(smem_quant2[off+126]), static_cast<int>(smem_quant2[off+127]));
                //     printf("quant_out_bdg_win[scale]: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f... %.4f, %.4f, %.4f, %.4f\n",
                //         smem_scale2[0], smem_scale2[1], smem_scale2[2], smem_scale2[3],
                //         smem_scale2[4], smem_scale2[5], smem_scale2[6], smem_scale2[7],
                //         smem_scale2[12], smem_scale2[13], smem_scale2[14], smem_scale2[15]);
                // }
