...
struct FusedMoeGemmPipelineFlatmmWRegPolicy
{
    ...
    {
        // using async
        constexpr index_t copy_bytes = 4 * Problem::AGmemLoadVectorLength; // same as ASmemStore
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_G()
    {
        ...
        if constexpr (PermuteEnum == FusedMoeGemmWeightPermuteEnum::no_permute) {
            ...
            return make_static_tile_distribution(
        ...
        else if constexpr(PermuteEnum == FusedMoeGemmWeightPermuteEnum::permute_b){
                using WarpGemm =
                ...
                constexpr index_t KVecotrLength = GetAlignment_G<Problem>();
                constexpr index_t KLane         = WarpGemm::Impl::kABKLane;
                constexpr index_t NLane       = get_warp_size() / KLane;
                constexpr index_t NInterleave = Problem::Gemm0NInterleave;
                constexpr index_t KRepeat     = Block_K / (KLane * KVecotrLength);
                static_assert(NInterleave * NLane * NumWarps == Block_N,
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_w4a16_G()
    {
        constexpr index_t Block_N  = Problem::BlockShape::Block_N0;
        constexpr index_t Block_K  = Problem::BlockShape::Block_K0 / Problem::PackFactor;  // 128/2=64 bytes
        constexpr index_t NumWarps = Problem::BlockShape::NumWarps;

        using WarpGemm = typename remove_cvref_t<decltype(GetWarpGemm0<Problem>())>::WarpGemmAttribute;
        constexpr index_t KLane         = WarpGemm::Impl::kABKLane;     // 4
        constexpr index_t KVecotrLength = GetAlignment_G<Problem>();    // 16 bytes, that leads to k elements number in gemm0 = 32. and this is needed by warp_gemm0

        constexpr index_t NLane       = get_warp_size() / KLane;        // 16
        constexpr index_t NInterleave = Problem::Gemm0NInterleave;
        static_assert(KVecotrLength * KLane == Block_K,
                    "GEMM0 tiling method in K direction is wrong!");
        static_assert(NInterleave * NLane * NumWarps == Block_N,
                    "GEMM0 tiling method in N direction is wrong!");
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_w4a16_G_SC()
    {
        constexpr index_t Block_N  = Problem::BlockShape::Block_N0;
        constexpr index_t Block_K  = Problem::BlockShape::Block_K0 / Problem::QuantBlockSizeK;  // 128/64=2
        constexpr index_t NumWarps = Problem::BlockShape::NumWarps;
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_w4a16_D()
    {
        constexpr index_t Block_N_  = Problem::BlockShape::Block_N1;
        constexpr index_t Block_K_  = Problem::BlockShape::Block_K1 / Problem::PackFactor;  // 128/2=64 bytes
        constexpr index_t NumWarps_ = Problem::BlockShape::NumWarps;

        constexpr index_t KVecotrLength = GetAlignment_D<Problem>();        // == 16, that leads to k elements number in gemm0 = 32. and this is needed by warp_gemm0
        using WarpGemm = typename remove_cvref_t<decltype(GetWarpGemm1<Problem>())>::WarpGemmAttribute;
        constexpr index_t KLane         = WarpGemm::Impl::kABKLane;
        static_assert(KLane * KVecotrLength == Block_K_,
                      "GEMM1 tiling method in K direction is wrong!");
        ...
        return make_static_tile_distribution(
            tile_distribution_encoding<sequence<1>,
                                       tuple<sequence<number<NumWarps_>{},
                                                      number<N_rep>{},
                                                      number<NLane>{},
                                                      number<NInterleave>{}>,   // <4,2,16,1>
                                             sequence<number<KLane>{}, number<KVecotrLength>{}>>,   // <4,16>
                                       tuple<sequence<1>, sequence<2, 1>>,
                                       tuple<sequence<0>, sequence<0, 2>>,
                                       sequence<1, 1, 2>,
                                       sequence<1, 3, 1>>{});
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsStoreDesc_A()
    {
        ...
        constexpr index_t KVector =
            GetAlignment_A<Problem>(); // choose 1/2/4 dword(must use 4 warps now)
        // TODO: support pad mode
        // constexpr index_t KPad    = KPack;                     // pad between warps

        static_assert(Block_K % KVector == 0);
        constexpr index_t LanesPerK = Block_K / KVector; // how many thread loading K 32/4 = 8
        ...
        else
        {
            ...
            constexpr index_t MLane     = warpSize / LanesPerK;         // along m  8
            constexpr index_t NumIssues = Block_M / (MLane * NumWarps); // dwordx2 1Issue
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsLoadDesc_A()
    {
        ...
        constexpr index_t KVector = Problem::ASmemLoadVectorLength;
        // constexpr index_t KPack   = GetSmemKPack_A<Problem>(); // LDS
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeBridgeTileDistribution()
    {
        ...
        constexpr auto bridge_tile_dstr_enc = tile_distribution_encoding<
            sequence<>,
            tuple<sequence<MRepeat, MWarps, MLane>, sequence<NWarps, kCN0PerLane, NLane, NVector>>,
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeBridgeLdsLoadDesc()
    {
        ...
        constexpr index_t NWarps = Problem::BlockShape::WarpPerBlock_N0;
        constexpr index_t MWarps = Problem::BlockShape::WarpPerBlock_M0;
        static_assert(NLane == 4 && kCN0PerLane == 4 && (NVector == 4 || NVector == 2 || NVector == 1));
        static_assert(NVector * NLane * kCN0PerLane == Problem::BlockShape::Warp_N0,
                      "Gemm0 bridge setting is wrong !");
        constexpr index_t MRepeat = Problem::BlockShape::Warp_M0 / MLane;
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetWarpGemm0()
    {
        ...
        // TODO: this is ugly
        if constexpr(Problem::FusedQuant == 0) {
            if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                        std::is_same_v<typename Problem::GDataType, ck_tile::half_t> &&
                        S_::Warp_M0 == 32 && S_::Warp_N0 == 64 && S_::Warp_K0 == 32)
            {
                return WarpGemmMmacF16F16F32_WT32x64x32_MR2NR1MI1NI4{};
            }
            else if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                            std::is_same_v<typename Problem::GDataType, ck_tile::half_t> &&
                            S_::Warp_M0 == 16 && S_::Warp_N0 == 32 && S_::Warp_K0 == 128 && 
                            Problem::Traits::PermuteEnum == FusedMoeGemmWeightPermuteEnum::no_permute)
            {
                return WarpGemmMmacF16F16F32_WT16x32x128_MR1NR1MI1NI2{};
            }
            else if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                            std::is_same_v<typename Problem::GDataType, ck_tile::half_t> &&
                            S_::Warp_M0 == 16 && S_::Warp_N0 == 32 && S_::Warp_K0 == 128 && 
                            Problem::Traits::PermuteEnum == FusedMoeGemmWeightPermuteEnum::permute_b)
            {
                return WarpGemmMmacF16F16F32_WT16x32x128_MR1NR1MI1NI2_Preshuffle{};
            }
        }
        else if constexpr (Problem::FusedQuant == 3)
        {
            if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                            std::is_same_v<typename Problem::GDataType, ck_tile::int8_t> &&
                            Problem::Traits::PermuteEnum == FusedMoeGemmWeightPermuteEnum::no_permute)
            {
                if constexpr(S_::Warp_M0 == 16 && S_::Warp_N0 == 32 && S_::Warp_K0 == 128)
                {
                    return WarpGemmMmacF16F16F32_WT16x32x128_MR1NR1MI1NI2{};
                }
                else if constexpr(S_::Warp_M0 == 16 && S_::Warp_N0 == 16 && S_::Warp_K0 == 128)
                {
                    return WarpGemmMmacF16F16F32_WT16x16x128_MR1NR1MI1NI1{};
                }
                            ...
                            Problem::Traits::PermuteEnum == FusedMoeGemmWeightPermuteEnum::no_permute)
            {
                if constexpr(S_::Warp_M0 == 16 && S_::Warp_N0 == 32 && S_::Warp_K0 == 128)
                {
                    return WarpGemmMmacBF16BF16F32_WT16x32x128_MR1NR1MI1NI2{};
                }
                else if constexpr(S_::Warp_M0 == 16 && S_::Warp_N0 == 16 && S_::Warp_K0 == 128)
                {
                    return WarpGemmMmacBF16BF16F32_WT16x16x128_MR1NR1MI1NI1{};
                }
            }
        }
    }
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetWarpGemm1()
    {
        ...
        if constexpr(Problem::FusedQuant == 0) {
            ...
            if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                        std::is_same_v<typename Problem::GDataType, ck_tile::half_t> &&
                            S_::Warp_M1 == 32 && S_::Warp_N1 == 16 && S_::Warp_K1 == 256)
            {
                return WarpGemmMmacF16F16F32_WT32x16x256_MR2NR1MI1NI1_TRANSC_LEGACY{};
            }
            else if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                            std::is_same_v<typename Problem::GDataType, ck_tile::half_t> &&
                            S_::Warp_M1 == 16 && S_::Warp_N1 == 16 && S_::Warp_K1 == 128)
            {
                if constexpr(Problem::IsSwizzled)
                {
                    return WarpGemmMmacF16F16F32_WT16x16x64_MR1NR1MI1NI1_TRANSC_LEGACY{};
                }
                else
                {
                    return WarpGemmMmacF16F16F32_WT16x16x128_MR1NR1MI1NI1_TRANSC_LEGACY{};
                }
            }
            else if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                            std::is_same_v<typename Problem::GDataType, ck_tile::half_t> &&
            ...
            {
                return WarpGemmMmacF16F16F32_WT16x32x128_MR1NR2MI1NI1_TRANSC_LEGACY{};
            }
        }
        else if constexpr (Problem::FusedQuant == 3)
        {
            if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::half_t> &&
                            std::is_same_v<typename Problem::GDataType, ck_tile::int8_t>)
            {
                if constexpr(S_::Warp_M1 == 16 && S_::Warp_N1 == 32 && S_::Warp_K1 == 128)
                {
                    return WarpGemmMmacF16F16F32_WT16x32x128_MR1NR2MI1NI1_TRANSC_LEGACY{};
                } 
                else if constexpr(S_::Warp_M1 == 16 && S_::Warp_N1 == 32 && S_::Warp_K1 == 64)
                {
                    return WarpGemmMmacF16F16F32_WT16x32x64_MR1NR2MI1NI1_TRANSC_LEGACY{};
                }
                                ...
                                std::is_same_v<typename Problem::GDataType, ck_tile::int8_t>)
            {
                if constexpr(S_::Warp_M1 == 16 && S_::Warp_N1 == 32 && S_::Warp_K1 == 128)
                {
                    return WarpGemmMmacBF16BF16F32_WT16x32x128_MR1NR2MI1NI1_TRANSC{};
                } 
                else if constexpr(S_::Warp_M1 == 16 && S_::Warp_N1 == 32 && S_::Warp_K1 == 64)
                {
                    return WarpGemmMmacBF16BF16F32_WT16x32x64_MR1NR2MI1NI1_TRANSC{};
                }
            }
        }
    }
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeBBlockTile_Gemm0()
    {
        using S_        = remove_cvref_t<typename Problem::BlockShape>;
        using WarpGemm  = remove_cvref_t<decltype(GetWarpGemm0<Problem>())>;
        ...
        constexpr auto b_block_outer_dstr_encoding =
            tile_distribution_encoding<sequence<>,
                                       tuple<sequence<S_::Repeat_N0, S_::WarpPerBlock_N0>,      // <1, 4>
                                             sequence<S_::Repeat_K0, S_::WarpPerBlock_K0>>,     // <1, 1>
                                       tuple<sequence<1, 2>>,
                                       tuple<sequence<1, 1>>,
                                       sequence<1, 2>,
                                       sequence<0, 0>>{};
        ...
        auto b_block_tensor         = make_static_distributed_tensor<BDataType>(b_block_dstr);
        return b_block_tensor;
    }

    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeBBlockTile_Gemm1()
    {
        using S_        = remove_cvref_t<typename Problem::BlockShape>;
        using WarpGemm  = remove_cvref_t<decltype(GetWarpGemm1<Problem>())>;
        using BDataType = typename WarpGemm::BDataType;
