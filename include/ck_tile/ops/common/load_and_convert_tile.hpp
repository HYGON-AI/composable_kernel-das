...
template <typename T>
struct is_packed_type<T, std::void_t<decltype(numeric_traits<remove_cvref_t<T>>::PackedSize)>>
    : std::bool_constant<(numeric_traits<remove_cvref_t<T>>::PackedSize > 1)>
{
};

template <typename T>
inline constexpr bool is_packed_type_v = is_packed_type<T>::value;
...
{
    using SrcDataType = typename WarpWindow::DataType;
