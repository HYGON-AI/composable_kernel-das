...
struct FusedConvIgemmPipelineWaspAgmemBgmemCregV2
{
                                   ...
                                   bool_constant<HasHotLoop>) const
    {
        ...
        // Load NumPrefetch - 1
        async_load_tile_wrapped_asm(in_gemm_lds_windows(number<NumPrefetch - 1>{}).get_buffer_ptr(),
                                    in_copy_dram_window);
        async_load_tile_wrapped_asm(
            wei_gemm_lds_windows(number<NumPrefetch - 1>{}).get_buffer_ptr(), wei_copy_dram_window);
        ...
        // read first
        promote_prio();
        auto in_warp_tensors  = blockwise_gemm.GetAWarpTensors(in_warp_windows_arr(number<0>{}));
        auto wei_warp_tensors = blockwise_gemm.GetBWarpTensors(wei_warp_windows_arr(number<0>{}));
        restore_prio();
        ...
        if constexpr(HasHotLoop)
        {
            ...
            do
            {
                ...
                static_for<0, NumPrefetch, 1>{}([&](auto i) {
                    ...
                    __builtin_amdgcn_sched_barrier(0);
                    async_load_fence((in_copy_dram_window.get_num_of_access() +
                    ...
                    in_warp_tensors = blockwise_gemm.GetAWarpTensors(
                        in_warp_windows_arr(number<(i + 1) % NumPrefetch>{}));
                    wei_warp_tensors = blockwise_gemm.GetBWarpTensors(
                        wei_warp_windows_arr(number<(i + 1) % NumPrefetch>{}));
                    restore_prio();
                });
        ...
        // tail
        {
            static_for<0, NumPrefetch, 1>{}([&](auto i) {
                ...
                if constexpr(i != NumPrefetch - 1)
                {
                    __builtin_amdgcn_sched_barrier(0);
                    if constexpr(NumPrefetch >= 3)
                    {
        ...
        if constexpr(HasHotLoop)
        {
            ...
            do
            {
                ...
                static_for<0, NumPrefetch, 1>{}([&](auto i) {
                    ...
                    // Read i
                    promote_prio();
                    auto in_warp_tensors  = blockwise_gemm.GetAWarpTensors(in_warp_windows_arr(i));
                    auto wei_warp_tensors = blockwise_gemm.GetBWarpTensors(wei_warp_windows_arr(i));
                    restore_prio();
                                   ...
                                   bool_constant<HasHotLoop>) const
    {
        ...
        if constexpr(HasHotLoop)
        {
            ...
            do
            {
                ...
                static_for<0, NumPrefetch, 1>{}([&](auto i) {
                    ...
                    __builtin_amdgcn_sched_barrier(0);
                    buffer_load_fence(wei_copy_dram_window.get_num_of_access() * (NumPrefetch - 1));
                    __builtin_amdgcn_sched_barrier(0);
        ...
        // tail
        {
            static_for<0, NumPrefetch, 1>{}([&](auto i) {
                __builtin_amdgcn_sched_barrier(0);
                buffer_load_fence(wei_copy_dram_window.get_num_of_access() * (NumPrefetch - 1 - i));
                __builtin_amdgcn_sched_barrier(0);
