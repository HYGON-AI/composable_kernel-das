...
// Avoid forming KThreadRead / 0 in an unselected constexpr conditional. HCU
// clang diagnoses that expression even when the ratio test selects the other
// branch. Specialization ensures the division is instantiated only for a
// strictly positive ratio greater than one.
template <index_t KThreadRead, index_t KThreadReadRatio, bool = (KThreadReadRatio > 1)>
struct safe_k_thread_read_perm
{
    static constexpr index_t value = KThreadRead;
};

template <index_t KThreadRead, index_t KThreadReadRatio>
struct safe_k_thread_read_perm<KThreadRead, KThreadReadRatio, true>
{
    static constexpr index_t value = KThreadRead / KThreadReadRatio;
};
...
struct UniversalGemmBasePolicy
{
    ...
    static constexpr auto GetATileAccessPattern()
    {
        if constexpr(has_a_tile_access_pattern<Derived>::value)
            ...
            return tile_distribution_pattern::thread_raked;
    }
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeALdsBlockDescriptor()
    {
        ...
        constexpr index_t MPerBlock = Problem::BlockGemmShape::kM;  //128
        constexpr index_t KPerBlock = Problem::BlockGemmShape::kK;  //64
        ...
        else if constexpr(Problem::Traits::SupportAColumnMajor &&
                          std::is_same_v<ALayout, tensor_layout::gemm::ColumnMajor>)
        {
            // A is loaded as a logical K x M tile. Transpose its register distribution before
            // storing to this Wave64 XOR-swizzled LDS descriptor so block GEMM sees M x K.
            ...
            constexpr auto KThreadWrite =
                TileEncodingPattern::Y0 * TileEncodingPattern::Y1;
            ...
            constexpr auto LdsBanksWidth = 32 * 4;
            constexpr auto kfold = (AK1 * M0 * sizeof(ADataType) > LdsBanksWidth)
                                       ? 1
                                       : LdsBanksWidth / (AK1 * M0 * sizeof(ADataType));
            constexpr index_t k_thread_read_ratio =
                static_cast<index_t>(kfold * K0PerThreadWrite / K0PerThreadRead);
            constexpr auto KThreadReadPerm =
                number<safe_k_thread_read_perm<static_cast<index_t>(KThreadRead),
                                               k_thread_read_ratio>::value>{};
            ...
            constexpr auto a_lds_block_desc_mk = transform_tensor_descriptor(
            ...
            return a_lds_block_desc_mk;
        }
        else
        {
            constexpr index_t KPack = GetSmemPackA<Problem>();      // kABKPerLane * kKIter = 4 * 1 = 4

            constexpr auto DataTypeSize = sizeof(ADataType);
            constexpr auto MLdsLayer =
                (32 * 4 / KPerBlock / DataTypeSize) < 1 ? 1 : (32 * 4 / KPerBlock / DataTypeSize);      // 32banks * 4bytes / 64 / 2 = 1

            constexpr auto a_lds_block_desc_0 = make_naive_tensor_descriptor(
                make_tuple(number<KPerBlock / KPack * MLdsLayer>{},     //16
                           number<MPerBlock / MLdsLayer>{},             //128
                           number<KPack>{}),                            //4 
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeBLdsBlockDescriptor()
    {
        ...
        else if constexpr(Problem::Traits::SupportBRowMajor &&
                          std::is_same_v<BLayout, tensor_layout::gemm::RowMajor>)
        {
            // B is loaded as a logical K x N tile. Keep that logical order in LDS while
            // applying the Wave64 XOR swizzle used by the upstream grouped GEMM policy.
            constexpr index_t BlockSize   = Problem::kBlockSize;
            constexpr index_t VecLoadSize =
        ...
        {
            constexpr index_t KPack     = GetSmemPackB<Problem>();          // kABKPerLane * kKIter = 4 * 1 = 4
            constexpr auto BK0          = number<KPerBlock / KPack>{};      // 16
            ...
            constexpr auto b_lds_block_desc_0 = make_naive_tensor_descriptor(
                make_tuple(
                    BK0 * number<NLdsLayer>{}, number<NPerBlock / NLdsLayer>{}, number<KPack>{}),       // <16, 128, 4>
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetGlobalVectorLoadSize()
    {
        ...
        constexpr index_t PackedSize =
            ck_tile::numeric_traits<remove_cvref_t<DataType>>::PackedSize;  //for half、fp8 = 1
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetVectorSizeC()
    {
        ...
        if constexpr(std::is_same_v<CLayout, tensor_layout::gemm::RowMajor>)
        {
            ...
            else
            {
                ...
                // return WG::WarpGemmAttribute::Impl::kCNLane / WG::kN;    // 4 / 32 = 0 for hg. 32/32=1 for amd.
                return 4;   //for hg gpu, it needs to be 1 or make Ninterleave available.   TODO.
            }
        }
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeADramTileDistribution()
    {
        ...
        constexpr index_t KPerBlock = Problem::BlockGemmShape::kK;  //64
        constexpr index_t VecLoadSize =
            Problem::FixedVectorSize ? Problem::VectorSizeA : GetVectorSizeA<Problem>();    //for half = 8
        constexpr index_t NumWaveGroups = Problem::NumWaveGroups;   //1

        // Tile: MPerBlock X KPerBlock
        if constexpr(std::is_same_v<ALayout, ck_tile::tensor_layout::gemm::RowMajor>)
        {
            using TileEncodingPattern = tile_distribution_encoding_pattern_2d<BlockSize,            //256
                                                                              MPerBlock,            //128
                                                                              KPerBlock,            //64
                                                                              VecLoadSize,          //8
                                                                              GetATileAccessPattern(),
...
// Compatibility policy used by the V3 quant pipeline. Keep the current main
// memory-transfer policy, but select the block GEMM through the HCU MMAC
// dispatcher.
    ...
    : public UniversalGemmBasePolicy<UniversalGemmPipelineAgBgCrPolicy>
{
    ...
    {
        return MmacUniversalGemmPipelineAgBgCrPolicy::template GetBlockGemm<Problem>();
    }
};
