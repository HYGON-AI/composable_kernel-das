...
struct BaseGemmPipelineAgBgCrMem
{
    ...
    // FIXME: 32KB is not the restriction for WASP pipeline, due to only 4wave in 12/8 wave read at the same time. So Prefetch Stage is only limited with LDS/VGPR size.
...
struct GemmPipelineAgBgCrMem : public BaseGemmPipelineAgBgCrMem<Problem>
{
    ...
    struct PipelineImpl<GemmPipelineScheduler::Intrawave>
    {
                                       ...
                                       void* p_smem) const
        {
            ...
            // TODO: support WASP, move window . LDS Direct can reuse this?
            ...
            if constexpr(HasHotLoop)
            {
                ...
                do
                {
                    static_for<0, PrefetchStages, 1>{}([&](auto prefetch_idx) {
                        ...
                        //TODO: support WASP, need tuple_array for a_lds_gemm_window
