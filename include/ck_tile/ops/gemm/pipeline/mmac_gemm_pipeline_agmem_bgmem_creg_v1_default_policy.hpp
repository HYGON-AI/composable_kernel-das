...
struct MmacGemmPipelineAGmemBGmemCRegV1DefaultPolicy
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeALdsBlockDescriptor()
    {
        ...
        constexpr index_t kMPerBlock = Problem::BlockGemmShape::kM;
        constexpr index_t kKPerBlock = Problem::BlockGemmShape::kK;
        //TODO: support more DS Vectorsize in Problem, now only support b128 or 8 element
        // constexpr auto a_lds_block_desc_0 = make_naive_tensor_descriptor(
        //     make_tuple(number<kKPerBlock / 8>{}, number<kMPerBlock>{}, number<8>{}), // K0=4 M=128 K1=8
        //     make_tuple(number<(kMPerBlock + 1) * 8>{}, number<8>{}, number<1>{}),
        //     number<8>{}, // scalarPerVector =8
        //     number<1>{});
        constexpr auto a_lds_block_desc_0 = make_naive_tensor_descriptor(
            make_tuple(number<kMPerBlock>{}, number<kKPerBlock / 8>{}, number<8>{}),
            make_tuple(number<kKPerBlock>{}, number<8>{}, number<1>{}),
            number<8>{},
            number<1>{});
        ...
        if constexpr(std::is_same_v<ALayout, ck_tile::tensor_layout::gemm::ColumnMajor>)
        {
            ...
            if constexpr(get_warp_size() % (K2 * M0))
            {
                ...
                return make_static_tile_distribution(
                    tile_distribution_encoding<sequence<1>, //replication dimension global emedding desc
                                               tuple<sequence<M0, M1>, sequence<K0, K1, K2, K3>>,
                                               tuple<sequence<2>, sequence<2, 1, 2>>, // cluster 
                                               tuple<sequence<0>, sequence<1, 0, 2>>, // arange order
                                               sequence<2, 1>,
                                               sequence<3, 1>>{});
            }
            else
    ...
    {
        using BlockGemmPolicy = MmacBlockGemmASmemBSmemCRegV1DefaultPolicy;
