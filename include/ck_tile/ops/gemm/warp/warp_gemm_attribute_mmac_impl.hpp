...
// gfx938 exposes the FP8/BF8 MMAC encodings through the lit/lts builtin family.
// Keep the legacy CK spellings used by the generic warp GEMM implementation,
// but select the plain (lit=false, lts=false) register layout.
#define __builtin_hcu_mmac_f32_16x16x32_fp8_fp8(a, b, c)                                           \
    __builtin_hcu_mmac_f32_16x16x32_fp8_fp8_lit_lts((a), (b), (c), false, false)
#define __builtin_hcu_mmac_f32_16x16x32_fp8_bf8(a, b, c)                                           \
    __builtin_hcu_mmac_f32_16x16x32_fp8_bf8_lit_lts((a), (b), (c), false, false)
#define __builtin_hcu_mmac_f32_16x16x32_bf8_fp8(a, b, c)                                           \
    __builtin_hcu_mmac_f32_16x16x32_bf8_fp8_lit_lts((a), (b), (c), false, false)
#define __builtin_hcu_mmac_f32_16x16x32_bf8_bf8(a, b, c)                                           \
    __builtin_hcu_mmac_f32_16x16x32_bf8_bf8_lit_lts((a), (b), (c), false, false)
#define __builtin_hcu_mmac_16x16x32_fp8_bf8(a, b, c)                                               \
    __builtin_hcu_mmac_f32_16x16x32_fp8_bf8_lit_lts((a), (b), (c), false, false)
#define __builtin_hcu_mmac_16x16x32_bf8_fp8(a, b, c)                                               \
    __builtin_hcu_mmac_f32_16x16x32_bf8_fp8_lit_lts((a), (b), (c), false, false)
#define __builtin_hcu_mmac_16x16x32_bf8_bf8(a, b, c)                                               \
    __builtin_hcu_mmac_f32_16x16x32_bf8_bf8_lit_lts((a), (b), (c), false, false)
#endif
...
struct WarpGemmAttributeMmacImplF16F16F32M16N16K16
{
    ...
    static constexpr index_t kCM1PerLane = kCMPerLane;
    // [HCU移植] BWD default policy (MakeLSEDLdsReadBlockDescriptor 等) 需要 lane 内 M0/M1 细分与
    //           N 每 lane 总数。basic(非转置) 布局每 lane 仅 1 个 M 元素(不再细分)，4 个 N 元素。
    static constexpr index_t kCM0PerLane = kCMPerLane;                // M 每 lane 总数=1
    static constexpr index_t kCNPerLane  = kCN0PerLane * kCN1PerLane; // N 每 lane 总数=4
    ...
    : public WarpGemmAttributeMmacImplF16F16F32M16N16K16
{
    ...
    // [HCU移植] TransC 路径：上游靠交换 A/B 操作数来转置 C；HCU MMAC 交换操作数并不能像
    // AMD MFMA 那样转置 C 寄存器布局。故 gfx938/gfx946 改调用 _lit_lts(lts=true) 内建函数
    // 实现转置；旧架构 gfx928/gfx936/gfx92a 仍走普通 mmac。
    // c_vec += a_vec * b_vec with LTS (lane transpose sum) to transpose C layout.
    // HCU basic MMAC operand swap does NOT change C register layout the same way
    // AMD MFMA does; use _lit_lts with lts=true on gfx938+ to achieve transposed C.
    CK_TILE_DEVICE void
    operator()(CVecType& c_vec, const AVecType& a_vec, const BVecType& b_vec) const
    {
#if defined(__gfx938__) || defined(__gfx946__)
        constexpr bool lit = false;
        constexpr bool lts = true;
        c_vec = __builtin_hcu_mmac_f32_16x16x16_f16_lit_lts(b_vec, a_vec, c_vec, lit, lts);
#elif defined(__gfx928__) || defined(__gfx936__) || defined(__gfx92a__)
...
// Fused-conv WASP kernels use conv/fused_conv TransC iterators whose C distribution
// matches the legacy operand-swap MMAC result. Keep that behavior separate from the
// gfx938/gfx946 lit_lts TransC path used by newer FMHA code.
struct WarpGemmAttributeMmacImplF16F16F32M16N16K16TransCLegacy
    : public WarpGemmAttributeMmacImplF16F16F32M16N16K16TransC
{
    using Base = WarpGemmAttributeMmacImplF16F16F32M16N16K16TransC;
...
struct WarpGemmAttributeMmacImplF16F16F32M16N16K16LitLts
{
    ...
    // mmac C layout (matches AMD MFMA: kCMLane=4, kCNLane=16)
    static constexpr index_t kCMLane = 4;
    static constexpr index_t kCNLane = 16;
    ...
    // kCMPerLane/kCN0PerLane/kCN1PerLane needed by base CWarpDstrEncoding / MakeCOutputLayout
...
struct WarpGemmAttributeMmacImplBf16Bf16F32M16N16K16
{
    ...
    static constexpr index_t kCN1PerLane = 1;
    static constexpr index_t kCM1PerLane = kCMPerLane;
    // [HCU移植] 同 F16 base：补 lane 内 M0 细分(=1)，供 BWD default policy 使用。
    static constexpr index_t kCM0PerLane = kCMPerLane;
...
// BF16 LIT LTS
// Composed of LIT and LTS
// [HCU移植] 修正：上游/早期 HCU 版本此处 A/B 误写成 fp16_t（从 fp16 结构体拷贝而来），
// bf16 LitLts 的输入类型应为 bf16_t，否则 bf16 FMHA 结果错误。
...
struct WarpGemmAttributeMmacImpl_f32_16x16x32_f8_base
{
    ...
    // MFMA-compatible aliases used by the quantized GEMM scale picker.
    static constexpr index_t kCM0PerLane = kCMPerLane;
    static constexpr index_t kCM1PerLane = 1;
    static constexpr index_t kCNPerLane  = kCN0PerLane * kCN1PerLane;
