...
struct QuantGemmKernel
{
    ...
    static constexpr bool kUsesMinBlockPerCuLaunchApi = true;
    ...
    struct SplitKBatchOffset
    {
                                     ...
                                     const std::size_t k_id = blockIdx.z)
        {
            ...
            else if constexpr(kQuantType == QuantType::ABQuantGrouped && !APreshuffleQuant)
            {
                ...
                {
                    bq_k_split_offset =
                        amd_wave_read_first_lane(bq_group_offset * kargs.stride_BQ);
                }
                else
                {
                    bq_k_split_offset = amd_wave_read_first_lane(bq_group_offset);
        ...
        const auto& bq_tensor_view = [&]() {
                              ...
                              kQuantType == QuantType::ABQuantGrouped)
            {
                ...
                else
                {
                    ...
                    {
                        constexpr bool StridedBQRowMajor =
                            kQuantType == QuantType::ABQuantGrouped &&
                            std::is_same_v<BLayout, tensor_layout::gemm::RowMajor>;
                        const auto bq_n = integer_divide_ceil(kargs.N, BQuantGroupSize::kN);
                        const auto bq_stride = StridedBQRowMajor ? kargs.stride_BQ : bq_n;
                        return make_naive_tensor_view<address_space_enum::global>(
                            bq_ptr,
                            make_tuple(kargs.QK_B - bq_group_offset, bq_n),
                            make_tuple(bq_stride, 1),
                    ...
                    else
                    {
                        constexpr bool DenseBQN =
                            ...
                            std::is_same_v<BLayout, tensor_layout::gemm::RowMajor>;
                        const auto bq_n =
                            DenseBQN ? kargs.N : integer_divide_ceil(kargs.N, BQuantGroupSize::kN);
                        return make_naive_tensor_view<address_space_enum::global>(
                            bq_ptr,
                            make_tuple(bq_n, kargs.QK_B - bq_group_offset),
        ...
        const auto& bq_block_window = [&]() {
                              ...
                              kQuantType == QuantType::ABQuantGrouped)
            {
                ...
                else
                {
                    ...
                    {
                        static_assert(std::is_same_v<BQLayout, tensor_layout::gemm::ColumnMajor>);
                        const auto bq_n_origin = DenseBQN ? i_n : i_n / BQuantGroupSize::kN;
                        return make_tile_window(
                            bq_tensor_view,
