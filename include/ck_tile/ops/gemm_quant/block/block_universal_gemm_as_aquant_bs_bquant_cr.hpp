...
#include "ck_tile/ops/gemm/block/mmac_block_gemm_asmem_bsmem_creg_v1_default_policy.hpp"
...
template <typename Problem_,
          typename Policy_     = MmacBlockGemmASmemBSmemCRegV1DefaultPolicy,
...
struct ABQuantBlockUniversalGemmAsBsCr : public BlockGemmQuantBase
{
    ...
    struct GemmTraits_
    {
        ...
        static constexpr bool DenseBQN =
...
#if CK_TILE_ARCH_SUPPORTS_INT4 || CK_TILE_ARCH_SUPPORTS_FP4
        static_assert(
                                       ...
                                       [[maybe_unused]] BSmemBlockWindow& b_block_window)
        {
            ...
            static_ford<sequence<MIterPerWarp, NIterPerWarp>>{}([&](auto mn) {
                ...
                static_for<0, Traits::QScalesPerBlockRow, 1>{}([&](auto kQScale) {
                    ...
                    tile_elementwise_inout([](auto& c) { c = 0; }, c_warp_tensor);
                    ...
                    constexpr auto c_block_y_index =
                        merge_sequences(sequence<mIter, nIter>{}, c_warp_y_index_zeros);
                    constexpr auto c_block_y_lengths =
                        merge_sequences(sequence<1, 1>{}, c_warp_y_lengths);
                    auto c_block_thread_buffer =
                        c_block_tensor.get_y_sliced_thread_data(c_block_y_index, c_block_y_lengths);
                    // a_scale
                    ...
                    if constexpr(BPreshuffleQuant)
                    {
                        ...
                        static_for<0, WarpGemm::kM * WarpGemm::kN / warp_size, 1>{}(
                            [&](auto c_row) {
                                ...
                                const float c_warp_value =
                                    c_warp_tensor.get_thread_buffer()[c_row];
                                c_block_thread_buffer[c_row] +=
                                    c_warp_value * a_scale_reg_f * b_scale_reg_f;
                            });
                    }
                    else
                    {
                        ...
                        constexpr index_t reg_offset = [&]() {
                            if constexpr(Traits::DenseBQN)
                            {
                                return nIter * Traits::KQPerBlock + kQScale;
                            }
                            else if constexpr(GemmTraits::BQuantGroupSize::kN >=
                                              (NWarp * WarpGemm::kN))
                        ...
                        static_for<0, WarpGemm::kM * WarpGemm::kN / warp_size, 1>{}(
                            [&](auto c_row) {
                                if constexpr(Traits::DenseBQN &&
                                             WarpGemm::WarpGemmAttribute::Impl::kCMLane == 16 &&
                                             (WarpGemm::kN == 32 || WarpGemm::kN == 64) &&
                                             Traits::KQPerBlock == 1)
                                {
                                    constexpr index_t kNPerLaneGroup =
                                        WarpGemm::kN /
                                        (warp_size /
                                         WarpGemm::WarpGemmAttribute::Impl::kCMLane);
                                    const index_t c_n = [&]() {
                                        if constexpr(WarpGemm::kN == 64)
                                        {
                                            // Match the final HCU CShuffle logical-N order. The
                                            // MMAC register groups and the 4x4 N sub-tile are
                                            // transposed relative to the intermediate CWarp view.
                                            constexpr index_t c_row_in_group = c_row % 16;
                                            return (get_lane_id() / 16) +
                                                   (c_row_in_group & 0x3) * 4 +
                                                   (c_row_in_group >> 2) * 16;
                                        }
                                        else
                                        {
                                            constexpr index_t c_n_in_group =
                                                (c_row % kNPerLaneGroup) / 2 +
                                                (c_row % 2) *
                                                    (kNPerLaneGroup / 2);
                                            const index_t c_n_base =
                                                (get_lane_id() /
                                                 WarpGemm::WarpGemmAttribute::Impl::kCMLane) *
                                                kNPerLaneGroup;
                                            return c_n_base + c_n_in_group;
                                        }
                                    }();
                                    const index_t src_lane_idx = [&]() {
                                        if constexpr(WarpGemm::kN == 64)
                                        {
                                            // The dense BQ distribution assigns scale N directly
                                            // to lane N for a 64-column HCU MMAC warp tile.
                                            return c_n;
                                        }
                                        else
                                        {
                                            // The 32-column distribution uses a 5-bit logical-N
                                            // permutation. Gather from its inverse lane.
                                            return ((c_n & 0x8) >> 3) |
                                                   ((c_n & 0x10) >> 3) | (c_n & 0x4) |
                                                   ((c_n & 0x1) << 3) |
                                                   ((c_n & 0x2) << 3);
                                    ...
                                    auto& dense_scale_reg =
                                        bq_block_tensor.get_thread_buffer()[reg_offset];
                                    uint32_t scale_reg_dword;
                                    if constexpr(std::is_same_v<BQDataType, float>)
                                    {
                                        scale_reg_dword =
                                            ck_tile::bit_cast<uint32_t>(dense_scale_reg);
                                    }
                                    else
                                    {
                                        scale_reg_dword = static_cast<uint32_t>(dense_scale_reg);
                                    }
                                    int gathered_scale_reg = __builtin_amdgcn_ds_bpermute(
                                        src_lane_idx << 2,
                    ...
                    c_block_tensor.set_y_sliced_thread_data(
                        c_block_y_index, c_block_y_lengths, c_block_thread_buffer);
                });
            });
        }
    };
