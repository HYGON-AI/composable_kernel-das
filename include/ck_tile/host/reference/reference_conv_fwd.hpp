...
struct ReferenceConvFwd
{
    ...
    struct Invoker
    {
        ...
        float Run(const Argument& arg)
        {
            if(!arg.packed_channel_)
            {
                ...
                if constexpr(NDimSpatial == 1)
                {
                    auto func = [&](auto g, auto n, auto k, auto wo) {
                        ...
                        {
                            for(std::size_t x = 0; x < arg.weight_.get_lengths()[3]; ++x)
                            {
                                auto wi = static_cast<long_index_t>(wo * arg.conv_strides_[0]) +
                                          static_cast<long_index_t>(x * arg.conv_dilations_[0]) -
                                          static_cast<long_index_t>(arg.in_left_pads_[0]);

                                if(wi >= 0 &&
                                   type_convert<std::size_t>(wi) < arg.input_.get_lengths()[3])
                                {
                                    float v_in;
                                    float v_wei;

                                    arg.in_element_op_(
                                        v_in, type_convert<float>(arg.input_(g, n, c, wi)));

                                    arg.wei_element_op_(
                                        v_wei, type_convert<float>(arg.weight_(g, k, c, x)));
                        ...
                        arg.output_(g, n, k, wo) = type_convert<OutDataType>(v_out);
                    };
                ...
                else if constexpr(NDimSpatial == 3)
                {
                    auto func = [&](auto g, auto n, auto k, auto d_o, auto ho, auto wo) {
                        ...
                        for(std::size_t c = 0; c < arg.weight_.get_lengths()[2]; ++c)
                        {
                            for(std::size_t z = 0; z < arg.weight_.get_lengths()[3]; ++z)
                            {
                                auto di = static_cast<long_index_t>(d_o * arg.conv_strides_[0]) +
                                          static_cast<long_index_t>(z * arg.conv_dilations_[0]) -
                                          static_cast<long_index_t>(arg.in_left_pads_[0]);
                                for(std::size_t y = 0; y < arg.weight_.get_lengths()[4]; ++y)
                                {
                                    auto hi =
                                        static_cast<long_index_t>(ho * arg.conv_strides_[1]) +
                                        static_cast<long_index_t>(y * arg.conv_dilations_[1]) -
                                        static_cast<long_index_t>(arg.in_left_pads_[1]);
                                    for(std::size_t x = 0; x < arg.weight_.get_lengths()[5]; ++x)
                                    {
                                        auto wi =
                                            static_cast<long_index_t>(wo * arg.conv_strides_[2]) +
                                            static_cast<long_index_t>(x * arg.conv_dilations_[2]) -
                                            static_cast<long_index_t>(arg.in_left_pads_[2]);
                                        if(di >= 0 &&
                                           type_convert<std::size_t>(di) <
                                               arg.input_.get_lengths()[3] &&
                                           hi >= 0 &&
                                           type_convert<std::size_t>(hi) <
                                               ...
                                               arg.input_.get_lengths()[5])
                                        {
                                            ...
                                            arg.wei_element_op_(
                                                v_wei,
                                                type_convert<float>(arg.weight_(g, k, c, z, y, x)));
                        ...
                        arg.output_(g, n, k, d_o, ho, wo) = type_convert<OutDataType>(v_out);
                    };
            ...
            {
                if((arg.input_.get_num_of_dimension() == NDimSpatial + 4 &&
                    arg.weight_.get_num_of_dimension() == NDimSpatial + 4 &&
                    arg.output_.get_num_of_dimension() == NDimSpatial + 4))
                {
                    if constexpr(NDimSpatial == 2)
                    {
                        auto func = [&](auto g, auto n, auto k, auto ho, auto wo, auto k_vect) {
                            float v_acc        = 0;
                            std::size_t K_VECT = arg.output_.get_lengths()[5];

                            for(std::size_t c = 0; c < arg.weight_.get_lengths()[2]; ++c)
                            {
                                for(std::size_t c_vect = 0; c_vect < arg.weight_.get_lengths()[5];
                                    ++c_vect)
                                {
                                    for(std::size_t y = 0; y < arg.weight_.get_lengths()[3]; ++y)
                                    {
                                        ...
                                        for(std::size_t x = 0; x < arg.weight_.get_lengths()[4];
                                            ++x)
                                        {
                                            auto wi =
                                                static_cast<long_index_t>(wo *
                                                                          arg.conv_strides_[1]) +
                                                static_cast<long_index_t>(x *
                                                   ...
                                                   arg.input_.get_lengths()[4])
                                            {
                                                ...
                                                arg.wei_element_op_(
                                                    ...
                                                    type_convert<float>(arg.weight_(
                                                        g, k * K_VECT + k_vect, c, y, x, c_vect)));
                            ...
                            arg.output_(g, n, k, ho, wo, k_vect) = type_convert<OutDataType>(v_out);
                        };
