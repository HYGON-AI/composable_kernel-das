...
template <typename BottomTensorView_,
          ...
          typename Layout_>
struct tile_window_conv2d_fwd_filter
{
    ...
    struct load_store_traits
    {
        ...
        static constexpr auto get_vector_dim_y_scalar_per_vector()
        {
            const auto [ys_vector_lengths, ys_vector_strides] =
                tile_window_conv2d_fwd_filter::get_window_adaptor_ys_safe_vector_length_strides();
    ...
    CK_TILE_DEVICE constexpr tile_window_conv2d_fwd_filter() = default;

    CK_TILE_DEVICE constexpr tile_window_conv2d_fwd_filter(
    ...
    CK_TILE_DEVICE void pre_compute_offsets_and_oob(const BottomTensorIndex& window_origin)
    {
        ...
        if constexpr(std::is_same_v<Layout_, tensor_layout::convolution::GKYXC>)
        {
            ...
            const auto K = bottom_tensor_view_.get_tensor_descriptor()
                               .get_transforms()[number<0>{}]
                               .get_upper_lengths()[number<0>{}];
            const auto Y = bottom_tensor_view_.get_tensor_descriptor()
                               .get_transforms()[number<0>{}]
                               .get_upper_lengths()[number<1>{}];
            const auto X = bottom_tensor_view_.get_tensor_descriptor()
                               .get_transforms()[number<0>{}]
        ...
        else if constexpr(std::is_base_of_v<tensor_layout::convolution::BaseGKCYXc, Layout_>)
        {
            // FIXME: hacky, assume tensor is in NHWC format, make sure origin tensor has 4 dims
            // TODO: can is_valid be computed directly by tensor desc?
            const auto K = bottom_tensor_view_.get_tensor_descriptor()
            ...
            static_for<0, NumAccess, 1>{}([&](auto iAccess) {
                ...
                const auto y  = bottom_tensor_thread_coord.get_hidden_index()[number<3>{}];
                const auto x  = bottom_tensor_thread_coord.get_hidden_index()[number<4>{}];
                const auto cx = bottom_tensor_thread_coord.get_hidden_index()[number<5>{}];

                // oob predicate computation, for padded desc
                if(k >= 0 && k < K && c >= 0 && c < C && y >= 0 && y < Y && x >= 0 && x < X &&
                   cx >= 0 && cx < Layout_::x)
                {
                    pre_computed_oob_(iAccess) = 1;
                }
    ...
    template <typename DistributedTensor, index_t i_access, bool oob_conditional_check = true>
...
make_tile_window_conv_fwd_filter(const TensorView_& tensor_view,
...
{
    return tile_window_conv2d_fwd_filter<remove_cvref_t<TensorView_>,
...
template <typename TensorView_,
          ...
          typename StaticTileDistribution_,
          typename Layout_>
CK_TILE_DEVICE void advance_tile(tile_window_conv2d_fwd_filter<TensorView_,
                                                               WindowLengths_,
                                                               FilterWindow_,
