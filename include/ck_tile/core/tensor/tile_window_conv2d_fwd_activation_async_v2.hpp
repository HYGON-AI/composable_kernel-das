...
struct tile_window_conv2d_fwd_activation_async_v2
                                        ...
                                        false>
{
    ...
    CK_TILE_DEVICE constexpr tile_window_conv2d_fwd_activation_async_v2(
    ...
    CK_TILE_DEVICE void init()
    {
        if constexpr(std::is_same_v<Layout_, tensor_layout::convolution::NHWGC>)
        {
            ...
            offset_delta_s_ = __builtin_amdgcn_readfirstlane(C);
            offset_delta_r_ = __builtin_amdgcn_readfirstlane(W * C - (S - 1) * C);
            offset_delta_c_ =
                __builtin_amdgcn_readfirstlane(TileShapeK - (R - 1) * W * C - (S - 1) * C);
                                       ...
                                       bool_constant<oob_conditional_check> = {}) const
    {
        ...
        auto issue = [&](auto ia) {
            ...
            {
                const bool is_src_valid = pre_computed_oob_[ia] & (uint16_t(1) << (samp_idx));
    ...
    CK_TILE_DEVICE void advance(number<next_samp_idx>)
    {
        ...
        else if constexpr((next_samp_idx % S) == 0)
        {
            move_mls_addr_base(buf_res_, offset_delta_r_ * sizeof(DataType));
        }
        else
        {
            move_mls_addr_base(buf_res_, offset_delta_s_ * sizeof(DataType));
        }
    }
...
make_tile_window_conv_fwd_activation_async_v2(const TensorView_& tensor_view,
                                              const TileShape_& window_lengths,
                                              const FilterSize_&,
                                              ...
                                              const Layout_&)
{
    if constexpr(std::is_same_v<Layout_, tensor_layout::convolution::NHWGC>)
    {
        return tile_window_conv2d_fwd_activation_async_v2<remove_cvref_t<TensorView_>,
