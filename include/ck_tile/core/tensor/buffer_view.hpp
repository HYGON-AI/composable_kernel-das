                   ...
                   Coherence>
{
    ...
    remove_cvref_t<T> invalid_element_value_ = T{0};
    index_t const_stride_;

    CK_TILE_HOST_DEVICE constexpr buffer_view()
        : p_data_{}, buffer_size_{}, cached_buf_res_{0}, invalid_element_value_{}, const_stride_{0}
    {
    }
    ...
    template <typename X,
              ...
              typename std::enable_if<
                  std::is_same<typename vector_traits<remove_cvref_t<X>>::scalar_type,
                               typename vector_traits<remove_cvref_t<T>>::scalar_type>::value,
                  bool>::type = false>
    CK_TILE_DEVICE constexpr auto get_by_inline_asm(index_t i,
                                                    index_t linear_offset,
                                                    bool is_valid_element,
                                                    bool_constant<oob_conditional_check> = {},
                                                    bool_constant<cache_swizzle>         = {}) const
    {
        // X contains multiple T
        constexpr index_t scalar_per_t_vector = vector_traits<remove_cvref_t<T>>::vector_size;
        ...
        if constexpr(cache_swizzle)
        {
            // TODO: how to ensure const_stride is pow2?
            return amd_buffer_load_inline_asm_invalid_element_return_zero<remove_cvref_t<T>,
                                                                          t_per_x,
                                                                          Coherence,
                                                                          oob_conditional_check>(
                p_data_, i + linear_offset, is_valid_element, buffer_size_, const_stride_);
        }
        else
        {
            return amd_buffer_load_inline_asm_invalid_element_return_zero<remove_cvref_t<T>,
    ...
    async_get_by_inline_asm(CK_TILE_LDS_ADDR remove_cvref_t<T>* smem,
                            index_t i,
                            index_t linear_offset,
                            bool is_valid_element,
                            bool_constant<oob_conditional_check> = {}) const
    {
        ...
        amd_async_buffer_load_inline_asm_with_oob<remove_cvref_t<T>, t_per_x, Coherence>(
            smem,
            cached_buf_res_,
            i,
            linear_offset,
...
#if defined(CK_TILE_GROUPED_GEMM_DSREADM_OUTPUT_MAPPED_IMMEDIATE_SCALAR_SHORT) || \
    ...
    // Store from one dynamic per-lane base plus a compile-time immediate.
    // The grouped-GEMM DSReadM epilogue uses this to match hipBLASLt's
    // buffer_store_short ... offset:<imm> address shape and avoid rebuilding a
    // tensor coordinate (and saving/restoring exec) for every scalar.
    template <typename X,
              index_t ImmediateElementOffset,
              bool oob_conditional_check = false,
              typename std::enable_if<
                  std::is_same<typename vector_traits<remove_cvref_t<X>>::scalar_type,
                               typename vector_traits<remove_cvref_t<T>>::scalar_type>::value,
                  bool>::type = false>
    CK_TILE_DEVICE void set_raw_immediate(index_t base_element_offset,
                                          bool is_valid_element,
                                          const X& x)
    {
        constexpr index_t scalar_per_t_vector =
            vector_traits<remove_cvref_t<T>>::vector_size;
        constexpr index_t scalar_per_x_vector =
            vector_traits<remove_cvref_t<X>>::vector_size;
        static_assert(scalar_per_x_vector % scalar_per_t_vector == 0);
        constexpr index_t t_per_x =
            scalar_per_x_vector / scalar_per_t_vector;
        constexpr index_t immediate_byte_offset =
            ImmediateElementOffset * sizeof(T);
        static_assert(immediate_byte_offset <= 0xfff);

        amd_buffer_store_raw_impl<remove_cvref_t<T>,
                                  t_per_x,
                                  Coherence,
                                  oob_conditional_check>(
            x,
            cached_buf_res_,
            base_element_offset * sizeof(T),
            0,
            immediate_byte_offset,
            is_valid_element);
    }
#endif

    // TODO: support buffer_atomic_pk_add in future
    template <typename X,
              typename std::enable_if<
                  std::is_same<typename vector_traits<remove_cvref_t<X>>::scalar_type,
                               typename vector_traits<remove_cvref_t<T>>::scalar_type>::value,
                   ...
                   amd_buffer_coherence_enum::coherence_default>
{
                                      ...
                                      bool_constant<oob_conditional_check> = {}) const
    {
        ...
        {
            auto rtn = *c_style_pointer_cast<const X*>(&p_data_[i + linear_offset]);
            return bit_cast<X>(rtn);
        }
    }
    ...
    // NOTE: get_raw is preferred.
    template <typename X,
              typename DsreadmInstr,
              typename std::enable_if<
                  std::is_same<typename vector_traits<remove_cvref_t<X>>::scalar_type,
                               typename vector_traits<remove_cvref_t<T>>::scalar_type>::value,
                  bool>::type = false>
    CK_TILE_DEVICE constexpr auto
    get_by_dsreadm(const DsreadmInstr& instr, index_t i, index_t linear_offset) const
    {
        // X contains multiple T
        constexpr index_t scalar_per_t_vector = vector_traits<remove_cvref_t<T>>::vector_size;
        ...
        return bit_cast<X>(instr(&p_data_[i + linear_offset]));
    }
    ...
    {
        smem_load<sizeof(X)>{}(
            dst, reinterpret_cast<uintptr_t>(p_data_) + v_offset * sizeof(T), i_offset * sizeof(T));
    }
    ...
    template <typename X,
              index_t PairLinearOffset,
              typename std::enable_if<
                  std::is_same<typename vector_traits<remove_cvref_t<X>>::scalar_type,
                               typename vector_traits<remove_cvref_t<T>>::scalar_type>::value,
                  bool>::type = false>
    CK_TILE_DEVICE void set_pair(index_t i, const X& x0, const X& x1)
    {
        constexpr index_t scalar_per_t_vector =
            vector_traits<remove_cvref_t<T>>::vector_size;
        constexpr index_t scalar_per_x_vector =
        ...
        static_assert(scalar_per_x_vector % scalar_per_t_vector == 0,
                      "X should contain multiple T");

        using buf_t =
            ext_vector_t<typename vector_traits<remove_cvref_t<T>>::scalar_type,
                         scalar_per_t_vector * scalar_per_x_vector>;

        // Keep both stores rooted at the same dynamic LDS address. For the
        // XOR4 CShuffle descriptor PairLinearOffset is 16 BF16 elements
        // (32 bytes), which maps directly to ds_write2_b64 offset1:4.
        *c_style_pointer_cast<buf_t*>(&p_data_[i]) =
            reinterpret_cast<const buf_t&>(x0);
        *c_style_pointer_cast<buf_t*>(&p_data_[i + PairLinearOffset]) =
            reinterpret_cast<const buf_t&>(x1);
    }
#endif
...
template <address_space_enum BufferAddressSpace,
          ...
          typename std::enable_if<std::is_same<remove_cvref_t<T>, remove_cvref_t<X>>::value,
                                  bool>::type = false>
CK_TILE_HOST_DEVICE constexpr auto
make_buffer_view(T* p, BufferSizeType buffer_size, X invalid_element_value, index_t const_stride)
{
    static_assert(BufferAddressSpace == address_space_enum::global);

    return buffer_view<BufferAddressSpace, T, BufferSizeType, false, Coherence>{
        p, buffer_size, invalid_element_value, const_stride};
}
