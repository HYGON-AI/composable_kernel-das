        ...
        else if constexpr(sizeof(S) == 1)
        {
            if constexpr(NX % 4 == 0 && NY % 4 == 0)
            {
                ...
                static_for<0, NY, 4>{}([&](auto iy) {
                    static_for<0, NX, 4>{}([&](auto ix) {
                        ...
                        vy_tuple(iy + I1).template get_as<S4>()(ix / I4) =
                            bit_cast<S4>(y_s4_1);
                        vy_tuple(iy + I2).template get_as<S4>()(ix / I4) =
                            bit_cast<S4>(y_s4_2);
                        vy_tuple(iy + I3).template get_as<S4>()(ix / I4) =
                            bit_cast<S4>(y_s4_3);
                    });
                });
            }
            else
            {
                static_for<0, NY, 1>{}([&](auto iy) {
                    static_for<0, NX, 1>{}(
                        [&](auto ix) { vy_tuple(iy)(ix) = vx_tuple[ix][iy]; });
                });
            }
        }
        else
