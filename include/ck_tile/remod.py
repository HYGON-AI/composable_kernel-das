        ...
        for k, v in self.m.items():
            if k == OPS:
                for km, kv in v.items():
                    gen_header(Path(k) / ('{}.hpp'.format(km)), kv)
            else:
                gen_header(Path('{}.hpp'.format(k)), v)
            

submodule = submodule_t()
# formatting
for x in all_files:
    subprocess.Popen('dos2unix {}'.format(str(x)), shell=True)
    cmd = 'clang-format-12 -style=file -i {}'.format(str(x))
    #for xp in x.parents:
