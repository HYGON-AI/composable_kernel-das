...
__host__ __device__ fp8_t run_cast_to_f8(T x, uint32_t rng)
{
    // check data type
    ...
    constexpr uint8_t ocp_inf_exp = (1 << f8_exp) - 1;
    constexpr uint8_t ocp_qnan    = 0x7F;
    ...
    {
        if((x_bitwise & nan_mask) == nan_mask)
            return mantissa == 0 ? fp8_t((sign << (f8_exp + f8_mant)) |
                                         (ocp_inf_exp << f8_mant))
                                 : fp8_t(ocp_qnan);
    }
    ...
    else if(exponent > max_exp)
    {
        ...
        {
            return fp8_t((sign << (f8_exp + f8_mant)) | (ocp_inf_exp << f8_mant));
        }
    }
...
__host__ __device__ T run_cast_from_f8(fp8_t x)
{
    // check data type
    constexpr bool is_half  = std::is_same<T, half_t>::value;
    constexpr bool is_float = std::is_same<T, float>::value;
    ...
    // unpack the input
    const uint8_t raw = static_cast<uint8_t>(x);
    uint32_t sign     = raw >> (f8_exp + f8_mant);
    uint32_t mantissa = raw & ((1 << f8_mant) - 1);
    int exponent      = (raw & 0x7F) >> f8_mant;

    if(exponent == 0 && mantissa == 0)
        return sign ? fNeg0 : static_cast<T>(0);
...
template <typename T, bool negative_zero_nan, bool clip, bool stoch>
__host__ __device__ fp8_t cast_to_f8(T x, uint32_t rng)
{
    // check datatype
    constexpr bool is_half  = std::is_same<T, half_t>::value;
    constexpr bool is_float = std::is_same<T, float>::value;
...
template <typename T, bool negative_zero_nan>
__host__ __device__ T cast_from_f8(fp8_t x)
{
    // check datatype
    constexpr bool is_half  = std::is_same<T, half_t>::value;
    constexpr bool is_float = std::is_same<T, float>::value;
