...
struct DynamicBuffer
{
    ...
    // for structured buffer with cache swizzle
    ...
    __host__ __device__ constexpr auto Get(index_t i, bool is_valid_element) const
    {
        ...
        if constexpr(GetAddressSpace() == AddressSpaceEnum::Global && use_amd_buffer_addressing)
        {
            ...
            if constexpr(InvalidElementUseNumericalZeroValue)
            {
                ...
                {
                    return amd_idx_buffer_load_invalid_element_return_zero<remove_cvref_t<T>,
                                                                           t_per_x,
                                                                           oob_check>(
                        p_data_, i, is_valid_element, element_space_size_, const_stride_);
                }
                else
                {
    ...
    template <typename DstBuffer, index_t NumElemsPerThread, bool UseIdxenLoad>
    __host__ __device__ void DirectCopyToLds(DstBuffer& dst_buf,
                                             index_t src_offset,
                                             index_t dst_offset,
                                             bool is_valid_element,
                                             uint8_t wave_lds_wrap_offset) const
    {
        ...
        hcu_direct_load_global_to_lds<T, NumElemsPerThread, UseIdxenLoad>(p_data_,
                                                                          src_offset,
                                                                          dst_buf.p_data_,
                                                                          dst_offset,
                                                                          is_valid_element,
    ...
    __host__ __device__ void AtomicAdd(index_t i, bool is_valid_element, const X& x)
    {
        ...
        bool constexpr use_hcu_buffer_addressing =
            CK_USE_HCU_BUFFER_ATOMIC_ADD_F32 ? is_same_v<remove_cvref_t<scalar_t>, float> : false;

        if constexpr(use_hcu_buffer_addressing && GetAddressSpace() == AddressSpaceEnum::Global)
        {
            constexpr index_t t_per_x = scalar_per_x_vector / scalar_per_t_vector;

            hcu_buffer_atomic_add<remove_cvref_t<T>, t_per_x>(
                x, p_data_, i, is_valid_element, element_space_size_);
        }
        else
        {
...
make_dynamic_buffer(T* p, ElementSpaceSize element_space_size, index_t const_stride)
{
    return DynamicBuffer<BufferAddressSpace, T, ElementSpaceSize, true>{
        p, element_space_size, const_stride, 0};
}
