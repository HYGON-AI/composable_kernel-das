...
#ifndef CK_DETAIL_LAMBDA_SRC_DST_ACCESS_UTIL_DEFINED
#define CK_DETAIL_LAMBDA_SRC_DST_ACCESS_UTIL_DEFINED
...
struct ThreadwiseTensorSliceTransfer_v3r1
{
    ...
    using SrcStorageData = typename storage_type<SrcData>::type;
    using DstStorageData = typename storage_type<DstData>::type;
          ...
          dst_element_op_(dst_element_op)
    {
        ...
        if constexpr(is_same<remove_cvref_t<SrcData>, remove_cvref_t<DstData>>::value &&
                     (is_same<remove_cvref_t<SrcData>, int4_t>::value ||
                      is_same<remove_cvref_t<SrcData>, uint4_t>::value))
        {
            // TODO: implement layout trans for packed i4/u4
            static_assert(SrcVectorDim == DstVectorDim);

            // TODO: implement more elementwise operation for packed i4/u4
            static_assert(is_same<remove_cvref_t<SrcElementwiseOperation>,
                                  remove_cvref_t<DstElementwiseOperation>>::value &&
                          is_same<remove_cvref_t<SrcElementwiseOperation>,
                            ...
                            Number<ThreadScratchId> thread_scratch_id = Number<ThreadScratchId>{})
    {
        ...
        static_assert(is_same<remove_cvref_t<typename SrcBuffer::type>,
                              remove_cvref_t<SrcStorageData>>::value,
                      "wrong! SrcBuffer and SrcStorageData data type are inconsistent");
        ...
        static_ford<decltype(ordered_src_access_lengths)>{}([&](auto ordered_src_access_idx) {
            ...
            using src_vector_type = vector_type_maker_t<SrcStorageData, SrcScalarPerVector>;
            ...
            static_for<0, SrcScalarPerVector, 1>{}([&](auto i) {
                SrcStorageData src_v;

                src_element_op_(src_v, src_vector_container.template AsType<SrcStorageData>()[i]);

                src_vector_container.template AsType<SrcStorageData>()(i) = src_v;
            });
    ...
    TransferDataFromSrcThreadScratchToDstThreadScratch(Number<ThreadScratchId> thread_scratch_id)
    {
        ...
        static_ford<SliceLengths>{}([&](auto idx) {
            ...
            dst_thread_scratch_(idx) =
                type_convert<SrcStorageData>(src_thread_scratch_tuple_[thread_scratch_id][idx]);
        });
#else
        ...
        else
        {
            static_ford<SliceLengths>{}([&](auto idx) {
                // convert from SrcData to DstData here
                dst_thread_scratch_(idx) =
                    type_convert<DstStorageData>(src_thread_scratch_tuple_[thread_scratch_id][idx]);
            });
        }
#endif
    }
                             ...
                             Number<ThreadScratchId> thread_scratch_id = Number<ThreadScratchId>{})
    {
        ...
        static_assert(is_same<remove_cvref_t<typename DstBuffer::type>,
                              remove_cvref_t<DstStorageData>>::value,
        ...
        static_ford<decltype(ordered_dst_access_lengths)>{}([&](auto ordered_dst_access_idx) {
            ...
            using dst_vector_type = vector_type_maker_t<DstStorageData, DstScalarPerVector>;
            ...
            static_for<0, DstScalarPerVector, 1>{}([&](auto i) {
                DstStorageData dst_v;

                // apply DstElementwiseOperation
                dst_element_op_(dst_v, dst_vector_container.template AsType<DstStorageData>()[i]);

                dst_vector_container.template AsType<DstStorageData>()(i) = dst_v;
            });
