...
struct ThreadGroupTensorSliceTransfer_LdsSwizzle_ConvFwdActivation
{
    ...
    __device__ constexpr ThreadGroupTensorSliceTransfer_LdsSwizzle_ConvFwdActivation(
...

    {
        ...
        PrecomputeSrcCoords(
            src_desc, src_block_slice_origin + thread_data_idx_begin, H, W, delta_h, delta_w);
                            ...
                            Number<ThreadScratchId> thread_scratch_id = Number<ThreadScratchId>{})
    {
        ...
        static_ford<decltype(thread_slice_lengths)>{}([&](auto ordered_access_idx) {
            ...
            if constexpr(oob_check)
            {
                ...
                const bool is_src_valid = oob_predicate_[idx] & (1 << step_id_);
    ...
    {
        constexpr auto step_id = Number<I % (ConvFilterR * ConvFilterS)>{};
