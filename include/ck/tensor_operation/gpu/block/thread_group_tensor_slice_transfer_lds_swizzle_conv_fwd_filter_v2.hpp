...
namespace lds_swizzle_conv_fwd_filter_detail {
...
struct ThreadGroupTensorSliceTransfer_LdsSwizzle_ConvFwdFilter_v2
{
    ...
    static constexpr auto thread_single_load_size =
        generate_sequence(lds_swizzle_conv_fwd_filter_detail::
    ...
    __device__ constexpr ThreadGroupTensorSliceTransfer_LdsSwizzle_ConvFwdFilter_v2(
...

    {
        ...
        PrecomputeSrcCoords(src_desc, src_block_slice_origin + thread_data_idx_begin);
        PrecomputeDstCoords(dst_desc, dst_block_slice_origin + thread_data_idx_begin);
    }

    __device__ void PrecomputeSrcCoords(const SrcDesc& src_desc, const Index& src_slice_origin_idx)
    {
        // FIXME: hacky, assume tensor is in NHWC format
        const auto K = src_desc.GetTransforms()[Number<0>{}].GetUpperLengths()[Number<0>{}];
        const auto Y = src_desc.GetTransforms()[Number<0>{}].GetUpperLengths()[Number<1>{}];
        const auto X = src_desc.GetTransforms()[Number<0>{}].GetUpperLengths()[Number<2>{}];
        ...
        static_ford<decltype(thread_slice_lengths)>{}([&](auto ordered_access_idx) {
            ...
            // FIXME: hacky, assume tensor is in NHWC format
            const auto k = src_coord.GetHiddenIndex()[Number<1>{}];
            const auto y = src_coord.GetHiddenIndex()[Number<2>{}];
            const auto x = src_coord.GetHiddenIndex()[Number<3>{}];
            const auto c = src_coord.GetHiddenIndex()[Number<4>{}];

            if(k >= 0 && k < K && y >= 0 && y < Y && x >= 0 && x < X && c >= 0 && c < C)
                oob_predicate_(idx) = 1;
                             ...
                             Number<ThreadScratchId> thread_scratch_id = Number<ThreadScratchId>{})
    {
        ...
        static_ford<decltype(thread_slice_lengths)>{}([&](auto ordered_access_idx) {
            ...
            static constexpr auto store_step = generate_sequence(
                lds_swizzle_conv_fwd_filter_detail::
...
template <typename ThreadGroup,
          ...
          bool bypass_l1,
          bool cache_swizzle>
struct ThreadGroupTensorSliceTransfer_LdsSwizzle_ConvFwdFilter_v2<ThreadGroup,
                                                                  BlockSliceLengths,
