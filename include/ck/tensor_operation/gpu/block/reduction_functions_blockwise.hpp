...
template <typename AccDataType,
          index_t BlockSize,
          typename ThreadClusterLengths_K0_M_K1,
          typename ThreadClusterArrangeOrder,
          typename OpReduce,
          bool PropagateNan,
          typename Accumulation =
              detail::AccumulateWithNanCheck<PropagateNan, OpReduce, AccDataType>>
struct PartitionedBlockwiseReductionV1R1
{
    static_assert(BlockSize == ThreadClusterLengths_K0_M_K1::At(1) * ThreadClusterLengths_K0_M_K1::At(4),
                  "The product of cluster lengths should be same as BlockSize!");

    static constexpr auto BufferLength_M = ThreadClusterLengths_K0_M_K1::At(1);
    static constexpr auto BufferLength_K = ThreadClusterLengths_K0_M_K1::At(4);

    static_assert(BufferLength_K > 1, "Parallel reduction need work on at least two elements");

    static constexpr auto block_buf_desc_m_k = make_naive_tensor_descriptor_packed(
        make_tuple(Number<1>{}, Number<BufferLength_M>{}, Number<1>{}, Number<1>{}, Number<BufferLength_K>{}));

    static constexpr auto thread_cluster_desc =
        make_cluster_descriptor(ThreadClusterLengths_K0_M_K1{}, ThreadClusterArrangeOrder{});
    ...
    __device__ static void Reduce(BufferType& work_buffer, AccDataType& in_out_value)
    {
        ...
        const auto thread_m_cluster_id = thread_cluster_idx[Number<1>{}];
        const auto thread_k1_cluster_id = thread_cluster_idx[Number<4>{}];

        // printf("cluster_len_shift:%d , thread_m_cluster_id:%d, thread_k1_cluster_id:%d \n", cluster_len_shift, thread_m_cluster_id, thread_k1_cluster_id);
        ...
        static_for<0, cluster_len_shift, 1>{}([&](auto I) {
            constexpr index_t indOffset = 1 << (cluster_len_shift - 1 - I());
            if(thread_k1_cluster_id < indOffset)
            {
                index_t offset1 = block_buf_desc_m_k.CalculateOffset(thread_cluster_idx);
                index_t offset2 = block_buf_desc_m_k.CalculateOffset(thread_cluster_idx +
                                                                     make_tuple(0, 0, 0 ,0 ,indOffset));
        ...
        index_t offset = block_buf_desc_m_k.CalculateOffset(make_tuple(0, thread_m_cluster_id, 0, 0, 0));
