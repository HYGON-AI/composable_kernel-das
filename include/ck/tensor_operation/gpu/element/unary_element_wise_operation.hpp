...
struct PassThrough
{
    ...
    template <>
    __host__ __device__ void operator()<uint4_t, uint4_t>(uint4_t& y, const uint4_t& x) const
    {
        y = x;
    }

    template <>
    __host__ __device__ void operator()<fp8_t, fp8_t>(fp8_t& y, const fp8_t& x) const
    {
        y = x;
    }

    // template <>
    // __host__ __device__ void operator()<float, fp8_t>(float& y, const fp8_t& x) const
    // {
    //     y = type_convert<float>(x);
    // }

    // template <>
    // __host__ __device__ void operator()<fp8_t, float>(fp8_t& y, const float& x) const
    // {
    //     y = type_convert<fp8_t>(x);
    // }

    // template <>
    // __host__ __device__ void operator()<half_t, fp8_t>(half_t& y, const fp8_t& x) const
    // {
    //     y = type_convert<half_t>(x);
    // }

    // template <>
    // __host__ __device__ void operator()<fp8_t, half_t>(fp8_t& y, const half_t& x) const
    // {
    //     y = type_convert<fp8_t>(x);
    // }
