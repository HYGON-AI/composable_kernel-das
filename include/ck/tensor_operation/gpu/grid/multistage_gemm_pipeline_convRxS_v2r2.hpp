...
struct MultiStageGemmPipeline_ConvRxS_v2r2
{
    template <bool v>
    ...
    __host__ __device__ static constexpr bool IsSupported(index_t num_loop)
    {
        ...
        if constexpr(loop_remainder == 0)
        {
            return num_loop % (NumPrefetch) == 0;
        }
        else
        {
            static_assert(loop_remainder == 1, "only support loop with 1 remainder");
                                  ...
                                  index_t num_loop)
    {
        ...
        if constexpr(HasMainKBlockLoop)
        {
            ...
            } while(loop < num_loop - loop_remainder - NumPrefetch);
        }
        ...
        // tail
        {
            static_for<0, NumPrefetch, 1>{}([&](auto i) {
                ...
                if constexpr((loop_remainder == 1) && (i == NumPrefetch - 1))
                {
                    // move to last
                    __builtin_amdgcn_sched_barrier(0);
                    a_blockwise_copy.Advance();
            ...
            if constexpr(loop_remainder == 1)
            {
                ...
                // Lds store
                a_blockwise_copy.RunWrite(
                    a_block_desc, a_block_bufs.At(Number<0>{}), Number<NumPrefetch - 1>{});
                b_blockwise_copy.RunWrite(
                    b_block_desc, b_block_bufs.At(Number<0>{}), Number<NumPrefetch - 1>{});
                __builtin_amdgcn_sched_barrier(0);
                                  ...
                                  index_t num_loop)
    {
        ...
        if constexpr(HasMainKBlockLoop)
        {
            ...
            do
            {
                ...
                static_for<0, NumPrefetch, 1>{}([&](auto i) {
                    ...
                    // DirectLoadIssuePerStage*(NumPrefetch - 1)
                    __builtin_amdgcn_sched_barrier(0);
                    vmcnt<blockwise_gemm.DirectLoadIssueA*(NumPrefetch - 1)>();
                    __builtin_amdgcn_sched_barrier(0);
        ...
        // tail
        {
            ...
            static_for<0, NumPrefetch, 1>{}([&](auto i) {
                // Wait num_loop-i-NumPrefetch
                __builtin_amdgcn_sched_barrier(0);
                vmcnt<blockwise_gemm.DirectLoadIssueA*(NumPrefetch - 1 - i)>();
                __builtin_amdgcn_sched_barrier(0);
