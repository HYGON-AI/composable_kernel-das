...
__launch_bounds__(GridwiseGemm::MaxBlockSize, MinimumOccupancy)
    ...
    kernel_gemm_xdl_cshuffle_v3_multi_abd(typename GridwiseGemm::Argument karg)
{
#if defined(__gfx9__) || defined(__gfx928__) || defined(__gfx92a__) || defined(__gfx936__) || \
    defined(__gfx938__) || defined(__gfx11__) || defined(__gfx12__)
    if constexpr(GridwiseGemm::template IsValidCompilationParameter<CGlobalMemoryDataOperation>())
    {
        ...
        auto p_as_grid =
            GridwiseGemm::AddAsGridPointerOffset(karg.p_as_grid,
                                                 splitk_batch_offset.a_k_split_offsets);
        auto p_bs_grid =
            GridwiseGemm::AddBsGridPointerOffset(karg.p_bs_grid,
                                                 splitk_batch_offset.b_k_split_offsets);
...
#endif
    // __attribute__((amdgpu_waves_per_eu(1, 1)))
    kernel_gemm_xdl_cshuffle_v3_multi_abd_2lds(typename GridwiseGemm::Argument karg)
{
#if defined(__gfx9__) || defined(__gfx928__) || defined(__gfx92a__) || defined(__gfx936__) || \
    defined(__gfx938__) || defined(__gfx11__) || defined(__gfx12__)
    if constexpr(GridwiseGemm::template IsValidCompilationParameter<CGlobalMemoryDataOperation>())
...
struct GridwiseGemmMultiABD_xdl_cshuffle_v3
          ...
          LargeTensors>
{
    ...
    static_assert((is_same_v<AElementwiseOperation, tensor_operation::element_wise::PassThrough> &&
                  ...
                  !DirectLoad);
    static_assert((NumATensor == 1 && NumBTensor == 1) || !DirectLoad,
                  "DirectLoad is only enabled for singleton A/B tuples");
    ...
    static constexpr index_t KPack =
        math::max(lcm_AK1_BK1, MfmaSelector<ComputeTypeA, MPerXdl, NPerXdl>::selected_mfma.k_per_blk);
    ...
    struct Problem
    {
        ...
        __host__ void Print() const
        {
            std::cout << "problem {" << "M:" << M << ", " << "N:" << N << ", " << "K:" << K << ", "
                      << "SC:" << StrideC << ", " << "MP:" << MPadded << ", "
                      << "NP:" << NPadded << ", " << "KRead:" << KRead << ", "
    ...
    struct SplitKBatchOffset
    {
        __device__ SplitKBatchOffset(Argument& karg, index_t k_id)
        {
            static_for<0, NumATensor, 1>{}([&](auto i) {
                if constexpr(is_same_v<tensor_layout::gemm::RowMajor, ALayout>)
                {
                    a_k_split_offsets[i] = k_id * karg.KRead;
                }
                else if constexpr(is_same_v<tensor_layout::gemm::ColumnMajor, ALayout>)
                {
                    a_k_split_offsets[i] = k_id * karg.KRead * karg.StrideAs[i];
                }
            });
            ...
            static_for<0, NumBTensor, 1>{}([&](auto i) {
                ...
                {
                    b_k_split_offsets[i] = k_id * karg.KRead * karg.StrideBs[i];
                }
                else if constexpr(is_same_v<tensor_layout::gemm::ColumnMajor, BLayout>)
                {
                    b_k_split_offsets[i] = k_id * karg.KRead;
                }
            });
        ...
        std::array<index_t, NumATensor> a_k_split_offsets;
        std::array<index_t, NumBTensor> b_k_split_offsets;
    };

    __device__ static auto
    AddAsGridPointerOffset(const AsGridPointer& p_as_grid,
                           const std::array<index_t, NumATensor>& offsets)
    {
        return generate_tuple([&](auto i) { return p_as_grid[i] + offsets[i]; },
                              Number<NumATensor>{});
    }

    __device__ static auto
    AddBsGridPointerOffset(const BsGridPointer& p_bs_grid,
                           const std::array<index_t, NumBTensor>& offsets)
    {
        return generate_tuple([&](auto i) { return p_bs_grid[i] + offsets[i]; },
                              Number<NumBTensor>{});
    }
    ...
    __device__ static bool constexpr IsValidCompilationParameter()
    {
        ...
        using MfmaInst = MfmaSelector<ComputeTypeA, MPerXdl, NPerXdl>;
    ...
    __host__ static constexpr bool CheckValidity(const Argument& karg)
    {
        ...
        if constexpr(NXdlPerWave % CShuffleNXdlPerWavePerShuffle != 0)
        {
            ...
            {
                std::cout << "GridwiseGemmMultiABD_xdl_cshuffle_v3 rejected: NXdlPerWave="
                          << NXdlPerWave
                          << " is not divisible by CShuffleNXdlPerWavePerShuffle="
                          << CShuffleNXdlPerWavePerShuffle << std::endl;
            }
            return false;
        }
        constexpr index_t ldsBufferCount =
        ...
        if(GetSharedMemoryNumberOfByteOnHost() * ldsBufferCount > get_lds_size())
        {
            if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
            {
                std::cout << "GridwiseGemmMultiABD_xdl_cshuffle_v3 rejected: LDS usage "
                          << GetSharedMemoryNumberOfByteOnHost() * ldsBufferCount
                          << " exceeds LDS size " << get_lds_size() << std::endl;
            }
            return false;
        }
        if(!is_xdl_wmma_k_supported<ComputeTypeA, KPerBlock>())
        {
            if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
            {
                std::cout << "GridwiseGemmMultiABD_xdl_cshuffle_v3 rejected: KPerBlock="
                          << KPerBlock << " is not supported by XDL/WMMA for ComputeTypeA"
                          << std::endl;
            }
            return false;
        }
        ...
        else
        {
            ...
            if((KReadPadSplited * (karg.KBatch - 1)) >= karg.K)
            {
                if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
                {
                    std::cout << "GridwiseGemmMultiABD_xdl_cshuffle_v3 rejected: padded split-K "
                                 "read range exceeds K. K="
                              << karg.K << ", KBatch=" << karg.KBatch
                              << ", KReadVec=" << KReadVec << std::endl;
                }
                return false;
            }
        }
        ...
        if constexpr(BlkGemmPipelineVer != BlockGemmPipelineVersion::v1)
        {
            if(num_k_loop <= BlockwiseGemmPipe::PrefetchStages)
            {
                if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
                {
                    std::cout << "GridwiseGemmMultiABD_xdl_cshuffle_v3 rejected: num_k_loop="
                              << num_k_loop << " must be greater than PrefetchStages="
                              << BlockwiseGemmPipe::PrefetchStages
                              << " for pipeline version enum value "
                              << static_cast<int>(BlkGemmPipelineVer)
                              << ". K=" << karg.K << ", KBatch=" << karg.KBatch
        ...
        if constexpr(!LargeTensors)
        {
                 ...
                 karg.M * karg.N * sizeof(CDataType) <= TwoGB))
            {
                if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
                {
                    std::cout << "GridwiseGemmMultiABD_xdl_cshuffle_v3 rejected: descriptor size "
                                 "exceeds 2GB in non-large-tensor mode"
                              << std::endl;
                }
                return false;
            }
                               ...
                               CElementwiseOperation c_element_op)
    {
        ...
        const index_t num_k_block_main_loop = __builtin_amdgcn_readfirstlane(
            (as_grid_desc_ak0_m_ak1[I0].GetLength(I0) *
             as_grid_desc_ak0_m_ak1[I0].GetLength(I2)) /
            KPerBlock);
