...
#include "ck/tensor_operation/gpu/grid/gridwise_gemm_mmac_tt_mk_n0k0k1n1_nm_v2_cshuffle.hpp"
...
struct DeviceGroupedConvBwdData_mmac_v2_cshuffle_mixed
...
{
    using DeviceOp = DeviceGroupedConvBwdData_mmac_v2_cshuffle_mixed;
    ...
    using GridwiseGemm = GridwiseGemm_mmac_tt_mk_n0k0k1n1_nm_v2_cshuffle<
        BlockSize,
        ADataType,
        BDataType,
        AccDataType,
              ...
              input_right_pads_{input_right_pads}
        {
            ...
            for(ck::index_t i_ytilde = 0; i_ytilde < YTilde; ++i_ytilde)
            {
                for(ck::index_t i_xtilde = 0; i_xtilde < XTilde; ++i_xtilde)
                {
                    ...
                    // Gemm A (dout)
                    const auto a_grid_desc_b_m_k = MakeAGridDescriptor_B_M_K(a_g_n_k_wos_lengths,
                                                                             a_g_n_k_wos_strides,
                                                                             b_g_k_c_xs_lengths,
                                                                             b_g_k_c_xs_strides,
                                                                             c_g_n_c_wis_lengths,
    ...
    static bool IsSupportedArgument(const Argument& arg)
    {
        ...
        {
            // TODO: investigate how to support stride != 1 with cshuffle bwd
            // check if it's stride=1 pad = 0 conv
            for(int i = 0; i < NDimSpatial; i++)
            {
        ...
        // Gemm A (doutput)
        if constexpr(is_same_v<ALayout, ctc::NHWGK>)
        {
    ...
    std::string GetTypeString() const override
    {
        ...
        // clang-format off
        str << "DeviceGroupedConvBwdData_mmac_v2_cshuffle_mixed"
            << "-"
            << BlockSize << "-"
            << MPerBlock << "-"
            << N0PerBlock << "-"
