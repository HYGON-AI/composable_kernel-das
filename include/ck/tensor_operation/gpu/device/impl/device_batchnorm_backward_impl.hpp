                                                          ...
                                                          NumBatchNormReduceDim>
{
              ...
              p_dbias_(p_dbias)
        {
            ...
            if(UseMultiblockInK)
            {
                ...
                while(true)
                {
                    ...
                    if(testBlkGroupSize <= (128 / MultiBlockFactor))
                        break;
    ...
    bool IsSupportedArgument(const BaseArgument* pArg) override
    {
        ...
        else
        {
            ...
            // specialized for NCHW bn spatial
            if constexpr(Rank == 4 && NumInvariantDim == 1)
            {
                // HW % vector_size == 0
                const auto HW = pArg_->xyLengths_[Rank - 1] * pArg_->xyLengths_[Rank - 2];
                if(HW % XSrcVectorSize != 0 || HW % DySrcVectorSize != 0 ||
                   HW % DxDstVectorSize != 0)
                    return false;
            }
            else
            {
    ...
    std::string GetTypeString() const override
    {
        ...
        // clang-format off
        str << "DeviceBatchNormBwdImpl<" << BlockSize << "_";
        str << "M_C" << MThreadClusterSize << "_S" << MThreadSliceSize << "_";
        str << "K_C" << KThreadClusterSize << "_S" << KThreadSliceSize << "_";
        ...
        str << "VectorSize_X" << XSrcVectorSize << "_scale_" << ScaleSrcVectorSize << "_bias_" << DscaleDbiasDstVectorSize << "_mean_var_" << MeanVarSrcVectorSize << "_Dx_" << DxDstVectorSize
            << "_multiBlock"<< UseMultiblockInK << "_factor" << MultiBlockFactor <<  ">";
        // clang-format on
