...
struct DeviceGemmMultiD_Xdl_CShuffle_V3 : public DeviceGemmV2R1<ALayout,
                                                                BLayout,
                                                                DsLayout,
                                                                CLayout,
                                                                ADataType,
                                                                ...
                                                                CElementwiseOperation>
{
    ...
    static bool IsSupportedArgument(const Argument& arg)
    {
        if(is_gfx11_supported() && arg.KBatch > 1)
        {
            ...
            {
                std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: gfx11 does not support "
                             "KBatch > 1"
                          << std::endl;
            }
            return false;
        ...
        if(!is_bf16_atomic_supported() && std::is_same_v<CDataType, ck::bhalf_t> && arg.KBatch > 1)
        {
            if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
            {
                std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: bf16 atomic is not "
                                                       ...
                                                       GemmSpec == GemmSpecialization::KPadding))
        {
            if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
            {
                std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: K=" << arg.K
        ...
        if(!partitioner.isPartitionable())
        {
            if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
            {
                std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: descriptors are not "
            ...
            if constexpr(NXdlPerWave64 > 0)
            {
                ...
                for(std::size_t i = 0; i < sub_arguments.size(); ++i)
                {
                    ...
                    if(!GridwiseGemm64::CheckValidity(sub_arg))
                    {
                        if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
                        {
                            std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: "
                    ...
                    if(!Partitioner::isDescriptorValidForGemm(sub_arg))
                    {
                        if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
                        {
                            std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: descriptor "
        ...
        if(CDEShuffleBlockTransferScalarPerVectors{}[Number<0>{}] <= 1 && (arg.KBatch > 1))
        {
            if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
            {
                std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: CDE shuffle vector for C "
                             "must be greater than 1 when KBatch > 1"
                          << std::endl;
            }
            return false;
        }
        ...
        if(ck::EnvIsEnabled(CK_ENV(CK_LOGGING)))
        {
            std::cout << "DeviceGemmMultiD_Xdl_CShuffle_V3 rejected: no valid warp-size path for "
