...
template <typename ADataType,
          ...
          bool PadN        = false,
          bool PadK        = false>
bool run_tensorwise_quant_gemm(const char* case_name,
                               ...
                               ck_tile::index_t K = 128)
{
    ...
    using Kernel   = ck_tile::
        QuantGemmKernel<TilePartitioner, Pipeline, Epilogue, ck_tile::QuantType::TensorQuant>;
    ...
    constexpr float a_scale = 0.5f;
    constexpr float b_scale = 1.25f;

    ck_tile::reference_gemm<ADataType, BDataType, AccDataType, float>(a, b, c_reference);
    for(std::size_t i = 0; i < c_expected.mData.size(); ++i)
    {
        c_expected.mData[i] =
            ck_tile::type_convert<CDataType>(c_reference.mData[i] * a_scale * b_scale);
    }
    ...
    ck_tile::DeviceMem a_scale_device(sizeof(float));
    ck_tile::DeviceMem b_scale_device(sizeof(float));

    a_device.ToDevice(a.data());
    b_device.ToDevice(b.data());
    c_device_buffer.SetZero();
    a_scale_device.ToDevice(&a_scale);
    b_scale_device.ToDevice(&b_scale);

    const ck_tile::QuantGemmHostArgs host_args{
        a_device.GetDeviceBuffer(),
        b_device.GetDeviceBuffer(),
        c_device_buffer.GetDeviceBuffer(),
        a_scale_device.GetDeviceBuffer(),
        b_scale_device.GetDeviceBuffer(),
        1,
        M,
        N,
        K,
    ...
    {
        std::cerr << "Tensorwise quant GEMM arguments are not supported" << std::endl;
        return false;
    }

    const dim3 grid  = Kernel::GridSize(M, N, 1);
    ...
    const auto elapsed_ms = ck_tile::launch_kernel(
        ...
        ck_tile::make_kernel<1>(Kernel{}, grid, block, 0, kernel_args));

    c_device_buffer.FromDevice(c_device.data());
    const bool pass =
        ck_tile::check_err(c_device, c_expected, "tensorwise quant GEMM mismatch", 1e-2, 1e-2);
...
int main(int argc, char* argv[])
{
    ...
    bool pass = true;
    pass = run_tensorwise_quant_gemm<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::half_t>(
               "tensorwise FP8xFP8->FP16") &&
           pass;
    if(run_bf16_output)
        pass = run_tensorwise_quant_gemm<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::bf16_t>(
                   "tensorwise FP8xFP8->BF16") &&
               pass;
    pass = run_tensorwise_quant_gemm<ck_tile::bf8_t, ck_tile::bf8_t, ck_tile::half_t>(
               "tensorwise BF8xBF8->FP16") &&
           pass;
    if(run_bf16_output)
        pass = run_tensorwise_quant_gemm<ck_tile::bf8_t, ck_tile::bf8_t, ck_tile::bf16_t>(
                   "tensorwise BF8xBF8->BF16") &&
               pass;
    pass = run_tensorwise_quant_gemm<ck_tile::fp8_t, ck_tile::bf8_t, ck_tile::half_t>(
               "tensorwise FP8xBF8->FP16") &&
           pass;
    if(run_bf16_output)
        pass = run_tensorwise_quant_gemm<ck_tile::fp8_t, ck_tile::bf8_t, ck_tile::bf16_t>(
                   "tensorwise FP8xBF8->BF16") &&
               pass;
    pass = run_tensorwise_quant_gemm<ck_tile::bf8_t, ck_tile::fp8_t, ck_tile::half_t>(
               "tensorwise BF8xFP8->FP16") &&
           pass;
    if(run_bf16_output)
        pass = run_tensorwise_quant_gemm<ck_tile::bf8_t, ck_tile::fp8_t, ck_tile::bf16_t>(
                   "tensorwise BF8xFP8->BF16") &&
               pass;
    if(!run_bf16_output)
        std::cout << "tensorwise BF16 output SKIP-PMD" << std::endl;
    pass = run_tensorwise_quant_gemm<ck_tile::fp8_t,
                                     ck_tile::fp8_t,
                                     ck_tile::half_t,
                                     Row,
                                     Row>("tensorwise FP8xFP8->FP16 RRR") &&
           pass;
    pass = run_tensorwise_quant_gemm<ck_tile::fp8_t,
                                     ck_tile::fp8_t,
                                     ck_tile::half_t,
                                     Col,
                                     Row>("tensorwise FP8xFP8->FP16 CRR") &&
           pass;
    pass = run_tensorwise_quant_gemm<ck_tile::fp8_t,
                                     ck_tile::fp8_t,
                                     ck_tile::half_t,
                                     ...
                                     true,
                                     true>(
               "tensorwise FP8xFP8->FP16 RCR padded 160x192x80", 160, 192, 80) &&
           pass;
    return pass ? 0 : 1;
}
