...
template <typename ADataType,
          ...
          bool PadN        = false,
          bool PadK        = false>
bool run_blockwise_quant_gemm(const char* case_name,
                              ...
                              ck_tile::index_t K = 256)
{
    ...
    constexpr ck_tile::index_t QuantGroupK = 128;
    ...
    using AQuantGroupSize =
        ck_tile::QuantGroupShape<ck_tile::sequence<1, 1, QuantGroupK>>;
    using BQuantGroupSize =
        ck_tile::QuantGroupShape<ck_tile::sequence<1, QuantGroupN, QuantGroupK>>;
    using Problem =
    ...
    const ck_tile::index_t AQK = ck_tile::integer_divide_ceil(K, QuantGroupK);
    const ck_tile::index_t BQK = ck_tile::integer_divide_ceil(K, QuantGroupK);
    const ck_tile::index_t BQN = ck_tile::integer_divide_ceil(N, QuantGroupN);
    ...
    const auto aq_desc =
        ck_tile::HostTensorDescriptor({std::size_t(M), std::size_t(AQK)},
                                      {std::size_t(AQK), std::size_t(1)});
    const auto bq_desc =
        ck_tile::HostTensorDescriptor({std::size_t(BQK), std::size_t(BQN)},
                                      {std::size_t(1), std::size_t(BQK)});
    ...
    for(ck_tile::index_t m = 0; m < M; ++m)
    {
        ...
        {
            aq(m, qk) = 0.25f + 0.01f * static_cast<float>(m % 7) +
    ...
    for(ck_tile::index_t m = 0; m < M; ++m)
    {
        for(ck_tile::index_t n = 0; n < N; ++n)
        {
            ...
            {
                const auto qk = k / QuantGroupK;
                const auto qn = n / QuantGroupN;
                acc += ck_tile::type_convert<float>(a(m, k)) *
                       ck_tile::type_convert<float>(b(k, n)) * aq(m, qk) * bq(qk, qn);
            }
            c_expected(m, n) = ck_tile::type_convert<CDataType>(acc);
        }
    }
    ...
    const bool pass =
        ck_tile::check_err(c_device, c_expected, "blockwise quant GEMM mismatch", 1e-2, 1e-2);
...
int main(int argc, char* argv[])
{
    ...
    bool pass = true;
    pass = run_blockwise_quant_gemm<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::half_t>(
               "blockwise FP8xFP8->FP16 RCR") &&
           pass;
    if(run_bf16_output)
        pass = run_blockwise_quant_gemm<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::bf16_t>(
                   "blockwise FP8xFP8->BF16 RCR") &&
               pass;
    pass = run_blockwise_quant_gemm<ck_tile::bf8_t, ck_tile::bf8_t, ck_tile::half_t>(
               "blockwise BF8xBF8->FP16 RCR") &&
           pass;
    if(run_bf16_output)
        pass = run_blockwise_quant_gemm<ck_tile::bf8_t, ck_tile::bf8_t, ck_tile::bf16_t>(
                   "blockwise BF8xBF8->BF16 RCR") &&
               pass;
    pass = run_blockwise_quant_gemm<ck_tile::fp8_t, ck_tile::bf8_t, ck_tile::half_t>(
               "blockwise FP8xBF8->FP16 RCR") &&
           pass;
    if(run_bf16_output)
        pass = run_blockwise_quant_gemm<ck_tile::fp8_t, ck_tile::bf8_t, ck_tile::bf16_t>(
                   "blockwise FP8xBF8->BF16 RCR") &&
               pass;
    pass = run_blockwise_quant_gemm<ck_tile::bf8_t, ck_tile::fp8_t, ck_tile::half_t>(
               "blockwise BF8xFP8->FP16 RCR") &&
           pass;
    if(run_bf16_output)
        pass = run_blockwise_quant_gemm<ck_tile::bf8_t, ck_tile::fp8_t, ck_tile::bf16_t>(
                   "blockwise BF8xFP8->BF16 RCR") &&
               pass;
    if(!run_bf16_output)
        std::cout << "blockwise BF16 output SKIP-PMD" << std::endl;
    pass =
        run_blockwise_quant_gemm<ck_tile::fp8_t,
                                 ck_tile::fp8_t,
                                 ck_tile::half_t,
                                 Row,
                                 Col,
                                 true,
                                 true,
                                 true>("blockwise FP8xFP8->FP16 RCR padded 160x192x160",
                                       160,
                                       192,
                                       160) &&
        pass;
    pass = run_blockwise_quant_gemm<ck_tile::fp8_t,
                                    ck_tile::fp8_t,
                                    ck_tile::half_t,
                                    Row,
                                    Row>("blockwise FP8xFP8->FP16 RRR") &&
           pass;
    pass = run_blockwise_quant_gemm<ck_tile::fp8_t,
                                    ck_tile::fp8_t,
                                    ck_tile::half_t,
                                    Col,
                                    Row>("blockwise FP8xFP8->FP16 CRR dense-BQ") &&
           pass;
    return pass ? 0 : 1;
}
