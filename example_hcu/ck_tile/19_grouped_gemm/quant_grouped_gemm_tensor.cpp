...
template <typename ADataType,
          ...
          typename ALayout,
          typename BLayout>
struct TensorwiseGroup
{
    TensorwiseGroup(ck_tile::index_t m,
                    ck_tile::index_t n,
                    ck_tile::index_t k,
                    float a_scale_value,
                    float b_scale_value)
        : M(m),
          ...
          aq_device(sizeof(float)),
          bq_device(sizeof(float))
    {
        ...
        aq_device.ToDevice(&a_scale);
        bq_device.ToDevice(&b_scale);
    }
    ...
    bool Check(std::size_t group_id)
    {
        ...
        return ck_tile::check_err(c_device,
                                  c_expected,
                                  "tensorwise quant grouped GEMM group " +
...
template <typename ADataType,
          ...
          bool PadN        = true,
          bool PadK        = true>
bool run_tensorwise_grouped_case(const char* case_name,
                                 ...
                                 const std::vector<ck_tile::index_t>& ks)
{
    ...
    using Kernel =
        ck_tile::QuantGroupedGemmKernel<TilePartitioner,
                                        ...
                                        Epilogue,
                                        ck_tile::QuantType::TensorQuant>;
    using Group = TensorwiseGroup<ADataType, BDataType, CDataType, ALayout, BLayout>;
    ...
    for(std::size_t i = 0; i < ms.size(); ++i)
    {
        groups.emplace_back(std::make_unique<Group>(
            ms[i], ns[i], ks[i], 0.40f + 0.05f * i, 0.65f + 0.03f * i));
...
bool run_tensorwise_dtype_matrix(const char* layout_name,
                                 const std::vector<ck_tile::index_t>& ms,
                                 const std::vector<ck_tile::index_t>& ns,
                                 const std::vector<ck_tile::index_t>& ks,
                                 bool run_bf16_output)
{
    const std::string prefix = "tensorwise grouped ";
    const std::string suffix = std::string{" "} + layout_name;
    bool pass                = true;

#define RUN_TENSORWISE_CASE(AType, BType, CType, TypeName)                                  \
    pass = run_tensorwise_grouped_case<AType, BType, CType, ALayout, BLayout>(              \
               (prefix + TypeName + suffix).c_str(), ms, ns, ks) &&                         \
           pass

    RUN_TENSORWISE_CASE(ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::half_t, "FP8xFP8->FP16");
    RUN_TENSORWISE_CASE(ck_tile::bf8_t, ck_tile::bf8_t, ck_tile::half_t, "BF8xBF8->FP16");
    RUN_TENSORWISE_CASE(ck_tile::fp8_t, ck_tile::bf8_t, ck_tile::half_t, "FP8xBF8->FP16");
    RUN_TENSORWISE_CASE(ck_tile::bf8_t, ck_tile::fp8_t, ck_tile::half_t, "BF8xFP8->FP16");
    if(run_bf16_output)
    {
        RUN_TENSORWISE_CASE(
    ...
    {
        std::cout << "tensorwise grouped BF16 output " << layout_name
                  << " SKIP-PMD" << std::endl;
    }

#undef RUN_TENSORWISE_CASE
    return pass;
}
...
int main(int argc, char* argv[])
{
    ...
    bool pass = run_tensorwise_dtype_matrix<Row, Col>(
        "NT fixed-K", fixed_ms, fixed_ns, fixed_ks, run_bf16_output);
    pass = run_tensorwise_dtype_matrix<Row, Row>(
               "NN fixed-K", fixed_ms, fixed_ns, fixed_ks, run_bf16_output) &&
           pass;
    pass = run_tensorwise_dtype_matrix<Col, Row>(
               "TN variable-K", variable_ms, variable_ns, variable_ks, run_bf16_output) &&
           pass;
    pass = run_tensorwise_grouped_case<ck_tile::fp8_t,
                                       ck_tile::fp8_t,
                                       ck_tile::half_t,
                                       Row,
                                       Col>("tensorwise grouped FP8xFP8->FP16 NT M/N/K padded",
                                            padded_ms,
                                            padded_ns,
                                            padded_ks) &&
           pass;
