...
auto create_args(int argc, char* argv[])
{
    ...
    parser.insert("tokens", "2", "token count")
        .insert("hidden", "96", "hidden dimension")
...
bool run(const ck_tile::ArgParser& parser)
{
    ...
    if(tokens != 2 || hidden != 96 || experts != 4 || topk != 2)
    {
        std::cerr << "this acceptance slice requires tokens=2, hidden=96, experts=4, topk=2"
                  << std::endl;
        return false;
    }
    ...
    using Shape = ck_tile::Generic2dBlockShape<ck_tile::sequence<2, 128>,
