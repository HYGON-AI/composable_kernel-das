                        ...
                        std::optional<std::string> json = std::nullopt)
{
    ...
    if constexpr(is_mx)
    {
#if defined(CK_TILE_FMHA_ENABLE_MX_TYPES)
...
#if defined(CK_TILE_FMHA_ENABLE_DROPOUT_REF)
        const float p_undrop = 1.0 - p_drop;
        const uint8_t p_undrop_in_uint8_t =
            uint8_t(std::floor(p_undrop * std::numeric_limits<uint8_t>::max()));
        const float rp_undrop = 1.0 / p_undrop;
#endif
...
#if defined(CK_TILE_FMHA_ENABLE_BLOCKSCALE_REF)
            else if(qscale.type == quant_scale_enum::blockscale)
            {
