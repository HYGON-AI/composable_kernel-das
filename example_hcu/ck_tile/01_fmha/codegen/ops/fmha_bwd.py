...
using fmha_bwd_shape_{F_idx} = ck_tile::TileFmhaBwdShape<fmha_block_tile_{F_idx},
                                                         ...
                                                         fmha_warp_tile2_{F_idx}>; // HCU移植: 旧 TileFmhaBwdShape 仅 11 个模板参，去掉上游新增的末位 maxq
...
using fmha_bwd_pipeline_problem_{F_idx} = ck_tile::BlockFmhaBwdPipelineProblem<
    ...
    fmha_mask_{F_idx},
    fmha_dropout_{F_idx},
    fmha_bwd_trait_{F_idx}>; // HCU移植: 旧 BlockFmhaBwdPipelineProblem 无 kUseTrLoad 模板参，去掉 trload 实参
...
// HCU移植: 旧 FmhaBwdDQDKDVKernel 仅 3 个模板参（pipeline + dK/dV epilogue）；
//          dQ 经 dq_acc + convert_dq 输出，不需要独立 dq epilogue，故去掉第 4 个实参。
...
{{
    ...
    // HCU移植: 旧 kernel 头无 kNeedsKernelPrezeroDqAcc / DqAccPrezeroKernel 内核预清零路径，
    //          dq_acc 预清零交由 launcher (fmha_bwd.hpp) 负责，这里直接单 kernel launch。
    return ck_tile::launch_kernel(
        s, ck_tile::make_kernel<CK_TILE_MAX_THREAD_PER_BLOCK, kBlockPerCu>(k_{{}}, grids, blocks, 0, kargs));
}}
...
{{
    ...
    constexpr ck_tile::index_t kBlockPerCu = k_::kBlockPerCu;
    // HCU移植: 同上，去掉内核预清零分支。
    ck_tile::make_kernel<CK_TILE_MAX_THREAD_PER_BLOCK, kBlockPerCu>(k_{{}}, grids, blocks, 0, kargs)(
        ck_tile::stream_config{{s.stream_id_}});
}}

// HCU移植: 旧 kernel 头不提供 kMaxSeqLenQ / GetWorkspace* / NeedsZeroDqAcc 等较新上游接口。
// 旧 HCU dq_dk_dv 流程把 dq 累加到外部 fp32 dq_acc 缓冲（即 workspace_ptr），再由 convert_dq
// 转回 dq。这里在 codegen 侧自包含实现这些 dispatch 桩，不触碰核心 kernel 头：
//   - maxq 概念已移除，返回编译期常量。
//   - 无 host 端 workspace 元数据，host_size / prepare 返回 0。
//   - device 端 workspace 即 dq_acc（fp32，按 dq 布局），需在 launch 前清零。
template <>
...
{{
    // dq_acc: fp32, [total_seqlen_q_padded, nhead_q, hdim_q] 布局上界（非确定性单 split）。
    return static_cast<size_t>(total_seqlen_q_padded) * static_cast<size_t>(nhead_q) *
           static_cast<size_t>(hdim_q) * sizeof(float);
}}
...
class KernelComponentFactoryGfx936(KernelComponentFactoryGfx9):
    ...
    @staticmethod
    def get_dq_dk_dv_tiles(dtype: str, tr_load: str) -> List[FmhaBwdDQDKDVTileSize]:
        # HCU移植: HCU MMAC 使用 16x16x32 (K=32) 和 16x16x16 (K=16) warp tile。
        # 上游 gfx9 的 d128 tile 的 bk2=16，但 G2 warp K=min(32,bk4)=32，
        # 导致 KIter=16/32=0.5 无效。此外 pipeline 约束 kM0==kK1, kM0==kK3，
        # 且 bk0/bk2 应等于 bhdq/bhdv 以保证 headdim 全量计算。
        # 此处提供 HCU 兼容的 tile 集合。
        if tr_load == "t":
            return []
        if dtype in ["fp16", "bf16"]:
            return [
                #                     bm0  bn0  bk0  bk1  bk2  bk3  bk4 bhdq bhdv
                # d64 tile (same as upstream, verified working on gfx938)
                FmhaBwdDQDKDVTileSize(  32, 128,  64,  32,  64,  32,  32,  64,  64, 1, 4, 1, 4, 1, 1, 1, 4, 1, 16, 16, 32, 16, 16, 16, 1),
                # d128 tile: bk0=bhdq=128, bk2=bhdv=128, bk1=bk3=kM0=16, bk4=32
                # G0(16,128,128) w16x16x32 KIter=4, G1(128,128,16) w16x16x16 KIter=1,
                # G2(16,128,128) w16x16x32 KIter=4, G3(128,128,16) w16x16x16 KIter=1,
                # G4(16,128,32)  w16x16x32 KIter=1, k4_loops=4
                FmhaBwdDQDKDVTileSize(  16, 128, 128,  16, 128,  16,  32, 128, 128, 1, 4, 1, 4, 1, 1, 1, 4, 1, 16, 16, 32, 16, 16, 16, 1),
            ]  # fmt: skip
        return []
    ...
    def supports_trait_combo(
        ...
        # HCU移植: 首轮 HCU BWD bring-up 只生成最小 batch/no-pad/no-dropout
        # 基础闭包。dropout/dbias/group/pad/trload 后续在 dQ/dK/dV 正确后逐个打开。
        return (
            dtype in ["fp16", "bf16"]
            and tr_load == "f"
            and mode == "batch"
...
class KernelComponentFactoryGfx946(KernelComponentFactoryGfx936):
    arch = ArchTrait("gfx946", preprocessor_check="defined(__gfx946__)", tag="void")
...
def get_factory(target: str):
    ...
    if target.startswith("gfx946"):
        return KernelComponentFactoryGfx946
...
using fmha_bwd_dot_do_o_pipeline_problem_{F_idx} = ck_tile::BlockFmhaBwdOGradDotOPipelineProblem<
    ...
    typename FmhaBwdTypeConfig<fmha_dtype_{F_idx}>::OGradDataType,
    typename FmhaBwdTypeConfig<fmha_dtype_{F_idx}>::DDataType,
    // HCU移植: 旧 BlockFmhaBwdOGradDotOPipelineProblem 无 LSEDataType 模板参，去掉。
    /* BlockSize = M0 = */ {F_bm0},
    {F_hdim},
    {F_mode},
...
using fmha_bwd_convert_dq_pipeline_problem_{F_idx} =
    ck_tile::BlockFmhaBwdConvertQGradPipelineProblem<
        ...
        /* BlockSize = */ 256,
        {F_bm0},
        // HCU移植: 旧 problem 需 kN0(=dq_dk_dv 的 k-seqlen tile bn0，用于确定性 dq_acc split 计数；
        //          非确定性路径不用到) 与 kQKHeaddim(=hdim) 两个独立参数。
        {F_bn0},
        {F_hdim},
        {F_mode},
        {F_deterministic},
...
class FmhaBwdConvertQGradKernel:
    ...
    F_dtype: str  # data type
    F_bm0: int  # tile size along q seqlen (block size)
    F_bn0: int  # HCU移植: dq_dk_dv 的 k-seqlen tile, 供旧 convert problem 的 kN0 使用
    F_spad: str  # true/false
    F_dpad: str  #
    F_mode: str  # value from MODE_MAP
    F_occupancy: int  #
        ...
        for tile, mode, mask, bias, dbias, dropout, spad1d, (
            ...
            if optdim_list != [-1]:
                if hdim not in optdim_list:
                    continue
            if not factory.supports_trait_combo(
                dtype,
                tr_load,
                mode,
                mask,
