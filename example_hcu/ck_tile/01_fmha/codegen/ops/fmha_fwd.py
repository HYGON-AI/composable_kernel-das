...
using fmha_tile_partitioner = ck_tile::FmhaFwdTilePartitioner_SHB<fmha_shape>;

using fmha_kernel = {F_kernel}<fmha_tile_partitioner, fmha_pipeline, fmha_epilogue>;
...
{{
    ...
    return ck_tile::launch_kernel(
        s, ck_tile::make_kernel<CK_TILE_MAX_THREAD_PER_BLOCK, kBlockPerCu>(
...
class FmhaFwdApiTrait:
    ...
    hdim_v_: str = ""  # problem V head_dim; falls back to bn1 in register_traits if empty
...
class FmhaFwdApiPool:
    ...
    def register_traits(self, trait: FmhaFwdApiTrait) -> None:
        hdim_v_key = int(trait.hdim_v_) if trait.hdim_v_ else trait.bn1
        hdim = trait.hdim, hdim_v_key
        ts = (
...
class FmhaFwdKernel:
    F_arch: ArchTrait
    F_hdim: int  # hdim
    F_hdim_v: int  # hdim_v (problem V head_dim)
...
class KernelComponentFactoryGfx936(KernelComponentFactoryGfx9):
    ...
    def get_hdim_tile_size_dict(cls, dtype: str) -> Optional[dict]:
        if dtype in cls._DT_FP16_BF16:
            return {
                # HCU MMAC bring-up tile:
                # gemm0(QK): 16x64x32 non-TransC, gemm1(PV): 16x32x64 TransC.
                (64, 64): [
                    FmhaFwdTileSize(
                        16,
                        64,
    ...
    def get_pipelines(
        ...
        if dtype not in cls._DT_FP16_BF16 or (hdim, hdim_v) not in (
            (64, 64),
            (128, 128),
        ):
            return []
...
class KernelComponentFactoryGfx928(KernelComponentFactoryGfx936):
    arch = ArchTrait("gfx928", preprocessor_check="defined(__gfx928__)", tag="void")


class KernelComponentFactoryGfx92a(KernelComponentFactoryGfx936):
    arch = ArchTrait("gfx92a", preprocessor_check="defined(__gfx92a__)", tag="void")
...
def get_factory(target: str):
    ...
    # Hygon HCU architectures — all share the same HCU MMAC tile config
    if target.startswith("gfx928"):
        return KernelComponentFactoryGfx928
    if target.startswith("gfx92a"):
        return KernelComponentFactoryGfx92a
