...
bool run_grouped_conv_fwd(const ck_tile::conv::ConvParam& conv_param, bool time_kernel)
{
    ...
    return ck_tile::check_err(out_device, out_host, "Error: incorrect results!", 1e-3f, 1e-3f);
...
bool run_grouped_conv_fwd_example(int argc, char* argv[])
{
    ...
    {
        return run_grouped_conv_fwd<2>(conv_param, arg_parser.get_bool("time"));
    }
    else
    {
