...
bool run(const ck_tile::ArgParser& args)
{
    ...
    ck_tile::HostTensor<XDataType> x0_host({m, n}, {stride, 1});
    ck_tile::HostTensor<XDataType> x1_host({m, n}, {stride, 1});
    ...
    if(validate && valid)
    {
        ...
        valid = ck_tile::check_err(y_host, y_ref, "Elementwise Add Error", 1e-3, 1e-3);
    }
