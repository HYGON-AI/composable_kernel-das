...
auto create_args(int argc, char* argv[])
{
    ...
    arg_parser.insert("m", "32", "input row count")
        .insert("n", "64", "input column count")
        .insert("stride_in", "-1", "input row stride; -1 means n")
...
bool run(const ck_tile::ArgParser& args)
{
    ...
    ck_tile::index_t stride_in = args.get_int("stride_in");
    if(stride_in < 0)
    {
        stride_in = n;
    }
    if(m <= 0 || n <= 0 || stride_in < n)
    {
        throw std::runtime_error("m/n must be positive and stride_in must be >= n");
    }
    ...
    using ElementWiseOperation = ck_tile::element_wise::PassThrough;
    ...
    const ck_tile::index_t stride_out = m;
    ck_tile::HostTensor<XDataType> x_host({m, n}, {stride_in, 1});
    ck_tile::HostTensor<YDataType> y_host({n, m}, {stride_out, 1});
    ck_tile::HostTensor<YDataType> y_ref({n, m}, {stride_out, 1});
    ...
    const auto input_strides  = ck_tile::make_tuple(stride_in, 1);
    const auto output_strides = ck_tile::make_tuple(1, stride_out);
    ...
    if(validate && valid)
    {
        ...
        for(ck_tile::index_t i = 0; i < m; ++i)
        {
            ...
            {
                y_ref(j, i) = x_host(i, j);
            }
        }
        valid = ck_tile::check_err(y_host, y_ref, "Elementwise Transpose Error", 0, 0);
    }
