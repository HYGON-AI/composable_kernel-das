...
static constexpr ck_tile::index_t NumLdsStages = 3;
...
bool run_fused_conv(const ck_tile::conv::ConvParam& conv_param, bool time_kernel)
{
    ...
    using Problem = ck_tile::FusedConvTlsWaspProblemV1<
        NDimSpatial,
    ...
    using Policy = ck_tile::FusedConvIgemmPipelineTlsWaspPolicyV1;

    using Pipeline = ck_tile::FusedConvIgemmPipelineTlsWaspAgmemBgmemCregV1<Problem, Policy>;
    ...
    using Kernel = ck_tile::FusedConvTlsWaspPersistantKernelV1<TilePartitioner, Pipeline, Epilogue>;
...
bool run_fused_conv_example(int argc, char* argv[])
{
    ...
    arg_parser.insert("m", "2", "spatial dim")
        ...
        .insert("k", "128", "out channels")
        .insert("c", "128", "in channels")
        .insert("r", "3", "filter x")
        .insert("s", "3", "filter y")
        .insert("h", "2", "input x")
        .insert("w", "2", "input y")
    ...
    {
        return run_fused_conv(conv_param, arg_parser.get_bool("time"));
    }
    else
    {
