...
using OutElementOp = ck_tile::element_wise::AddRelu;
...
#else
static constexpr auto ConvSpec = ck_tile::ConvFwdSpec::Filter3x3;
#endif
...
bool run_fused_conv(const ck_tile::conv::ConvParam& conv_param)
{
    ...
    ck_tile::FillUniformDistribution<BiasDataType>{-.0001f, 0.0001f}(bias);
#else
    ck_tile::FillUniformDistribution<InDataType>{1.0f, 1.0f}(in);
    ck_tile::FillUniformDistribution<WeiDataType>{1.0f, 1.0f}(wei);
    ck_tile::FillUniformDistribution<BiasDataType>{0.0f, 0.0f}(bias);
#endif

    if constexpr(FuseMode == ck_tile::FusedConvMode::ConvBiasResRelu)
    {
        ck_tile::FillUniformDistribution<OutDataType>{0.0001f, 0.0001f}(res);
    }
    else
    {
        ck_tile::FillUniformDistribution<OutDataType>{0.0f, 0.0f}(res);
    }
    ...
    using TileShape = ck_tile::FusedConvWaspTileShapeV1<decltype(WGs),
                                                        decltype(WGTile),
                                                        decltype(WGWarps),
                                                        decltype(WarpTile)>;
    ...
    using Problem = ck_tile::FusedConvWaspProblemV1<
        NDimSpatial,
    ...
    using WarpGemm = typename ck_tile::conv::WarpGemmMmacDispatcher<InDataType,
                                                                    WeiDataType,
                                                                    AccDataType,
                                                                    MmmacIter,
                                                                    NmmacIter,
                                                                    ...
                                                                    true>;
#if 1
    using Policy = ck_tile::FusedConvIgemmPipelineWaspPolicyV2R1;

    using Pipeline = ck_tile::FusedConvIgemmPipelineWaspAgmemBgmemCregV2R1<Problem, Policy>;

    using Epilogue = ck_tile::FusedConvEpilogueWaspV1<Problem, WarpGemm>;

    using TilePartitioner = ck_tile::ConvIgemmTilePartitioner<TileShape>;

    using Kernel =
        ck_tile::FusedConvWaspKernelV2R1<TilePartitioner, Pipeline, Epilogue, ConvSpec, FuseMode>;
#else

    using Policy = ck_tile::FusedConvIgemmPipelineWaspPolicyV1;

    using Pipeline = ck_tile::FusedConvIgemmPipelineWaspAgmemBgmemCregV1<Problem, Policy>;
    ...
    using Kernel =
        ck_tile::FusedConvWaspKernelV1<TilePartitioner, Pipeline, Epilogue, ConvSpec, FuseMode>;
#endif
...
bool run_fused_conv_example(int argc, char* argv[])
{
    ...
    arg_parser.insert("m", "2", "spatial dim")
        .insert("g", "1", "group")
        .insert("n", "256", "batch size")
        .insert("k", "256", "out channels")
        .insert("c", "1024", "in channels")
        .insert("r", "1", "filter x")
        .insert("s", "1", "filter y")
        .insert("h", "14", "input x")
        .insert("w", "14", "input y")
