...
#if defined(CK_TILE_MLS_PROBE_BF16)
...
static constexpr ck_tile::index_t MWGs = 1;
static constexpr ck_tile::index_t NWGs = 2;
...
static constexpr ck_tile::index_t OutGmemStoreVecLen = 8;
...
#if defined(CK_ENABLE_HCU_GFX946_INSTANCES)
static constexpr auto HcuArch = ck_tile::hcu_target_enum::gfx946;
#else
static constexpr auto HcuArch = ck_tile::hcu_target_enum::gfx938;
#endif

bool run_fused_conv(const ck_tile::conv::ConvParam& conv_param, bool time_kernel)
{
...
#if 1
    // Keep the validation signal well above fp16 round-off.  The former tiny
    // weights could let a missing/zero MLS load pass a loose all-close check.
    ck_tile::FillUniformDistribution<InDataType>{0.25f, 1.0f}(in);
    ck_tile::FillUniformDistribution<WeiDataType>{0.125f, 0.5f}(wei);
...
#else
    ck_tile::FillUniformDistribution<InDataType>{1.0f, 1.0f}(in);
    ck_tile::FillUniformDistribution<WeiDataType>{1.0f, 1.0f}(wei);
    ck_tile::FillUniformDistribution<BiasDataType>{-.0000f, 0.0000f}(bias);
    ck_tile::FillUniformDistribution<OutDataType>{0.0000f, 0.0000f}(res);
#endif
    ...
    using Problem = ck_tile::FusedConvMlsWaspProblemV1<
        NDimSpatial,
    ...
    using Policy = ck_tile::FusedConvIgemmPipelineMlsWaspPolicyV1;

    using Pipeline = ck_tile::FusedConvIgemmPipelineMlsWaspAgmemBgmemCregV1<Problem, Policy>;

    using TilePartitioner = ck_tile::ConvIgemmTilePartitioner<TileShape>;

    using BlockwiseGemm = ck_tile::remove_cvref_t<
        decltype(Policy::template GetBlockwiseGemm<HcuArch, Problem>())>;
    ...
    using Kernel =
        ck_tile::FusedConvMlsWaspKernelV1<HcuArch, TilePartitioner, Pipeline, Epilogue>;
...
#if defined(CK_TILE_MLS_PROBE_PERF_ONLY)
    return true;
#else
    // cpu fused conv
    ...
    auto ref_arg        = ref_fused_conv.MakeArgument(in,
                                               ...
                                               WeiElementOp{},
                                               OutElementOp{});
    ref_invoker.Run(ref_arg, false); // skip relu
    ...
    return ck_tile::check_err(
        out_device, out_host, "Error: incorrect results!", tolerance, tolerance);
#endif
}
...
bool run_fused_conv_example(int argc, char* argv[])
{
    ...
    {
        return run_fused_conv(conv_param, arg_parser.get_int("time") != 0);
    }
    else
    {
