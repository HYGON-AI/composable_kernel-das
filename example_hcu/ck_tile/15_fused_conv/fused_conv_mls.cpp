...
// configs
static constexpr ck_tile::index_t MPerBlock = 128;
...
bool run_fused_conv(const ck_tile::conv::ConvParam& conv_param, bool time_kernel)
{
    ...
    using Pipeline = ck_tile::FusedConvIgemmPipelineMlsAgmemBgmemCregV1<Problem, Policy>;

    using TilePartitioner = ck_tile::ConvIgemmTilePartitioner<TileShape>;

    using Kernel = ck_tile::FusedConvMlsKernelV1<TilePartitioner, Pipeline, ConvSpec, FuseMode>;
    ...
    float avg_time =
        ck_tile::launch_kernel({nullptr, time_kernel, 0, 0, 1},
