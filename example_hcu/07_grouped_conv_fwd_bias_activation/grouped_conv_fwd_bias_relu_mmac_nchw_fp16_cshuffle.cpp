...
#include "ck/tensor_operation/gpu/device/impl/device_grouped_conv_fwd_bias_activation_mmac_nchw_v2_cshuffle.hpp"
...
using InLayout  = ctc::NGCHW;
using WeiLayout = ctc::GKCYX;
using OutLayout = ctc::NGKHW;
using DeviceGroupedConvInstance =
    ck::tensor_operation::device::DeviceGroupedConvFwdBiasActivation_mmac_nchw_v2_cshuffle<
        2,
        InLayout,
        WeiLayout,
        OutLayout,
        ...
        S<1, 1, 1, 16, 16>,
        4, // a vector dim
        2, // a vector len: one DWORD per gfx946 direct-load thread
#else
        S<1, 4, 1, 16, 4>,
        4, // a vector dim
        8, // a vector len
...
bool run_grouped_conv_fwd_example(int argc, char* argv[])
{
    ...
    ck::utils::conv::ConvParam conv_param{
        2, 1, 1, 16, 16, {1, 1}, {4, 4}, {1, 1}, {1, 1}, {0, 0}, {0, 0}};
