...
// input FP16 scale/bias/mean FP32
...
// input BF16 scale/bias/mean FP32
                                                     ...
                                                     NumReduceDim>>
{
    ...
    static auto GetInstances()
    {
        ...
        else if constexpr(is_same_v<XDataType, F16> && is_same_v<YDataType, F16> &&
                          is_same_v<AccDataType, F32> && is_same_v<ScaleDataType, F32> &&
                          is_same_v<BiasDataType, F32> && is_same_v<MeanVarDataType, F32>)
        {
            if constexpr(Rank == 4 && NumReduceDim == 3 && is_same_v<YElementwiseOp, PassThrough>)
            {
                add_device_batchnorm_forward_rank_4_3_f16_scale_f32_instances(op_ptrs);
            }
        }
                          ...
                          is_same_v<BiasDataType, F32> && is_same_v<MeanVarDataType, F32>)
        {
            if constexpr(Rank == 4 && NumReduceDim == 3 && is_same_v<YElementwiseOp, PassThrough>)
            {
                add_device_batchnorm_forward_rank_4_3_bf16_scale_f32_instances(op_ptrs);
            }
        }
