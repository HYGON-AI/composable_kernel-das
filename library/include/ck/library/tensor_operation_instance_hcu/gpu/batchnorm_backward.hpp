...
void add_device_batchnorm_backward_rank_4_3_f16_scale_f32_instances(
    std::vector<std::unique_ptr<
        DeviceBatchNormBwd<F16, F16, F16, F32, F32, F32, F32, PassThrough, 4, 3>>>&);

void add_device_batchnorm_backward_rank_4_3_f16_instances(
    std::vector<std::unique_ptr<
        DeviceBatchNormBwd<F16, F16, F16, F32, F16, F16, F32, PassThrough, 4, 3>>>&);
...
void add_device_batchnorm_backward_rank_4_3_bf16_scale_f32_instances(
    std::vector<std::unique_ptr<
        DeviceBatchNormBwd<BF16, BF16, BF16, F32, F32, F32, F32, PassThrough, 4, 3>>>&);
                                                     ...
                                                     NumReduceDim>>
{
    ...
    static auto GetInstances()
    {
        ...
        if constexpr(is_same_v<XDataType, F16> && is_same_v<DxDataType, F16> &&
                     is_same_v<DyDataType, F16> && is_same_v<AccDataType, F32> &&
                     is_same_v<ScaleDataType, F16> && is_same_v<DscaleDbiasDataType, F16> &&
        ...
        else if constexpr(is_same_v<XDataType, F16> && is_same_v<DxDataType, F16> &&
                          is_same_v<DyDataType, F16> && is_same_v<AccDataType, F32> &&
                          ...
                          is_same_v<MeanVarDataType, F32>)
        {
            if constexpr(Rank == 4 && NumReduceDim == 3 && is_same_v<DyElementwiseOp, PassThrough>)
            {
                add_device_batchnorm_backward_rank_4_3_f16_scale_f32_instances(op_ptrs);
            }
        }
        ...
        else if constexpr(is_same_v<XDataType, BF16> && is_same_v<DxDataType, BF16> &&
                          is_same_v<DyDataType, BF16> && is_same_v<AccDataType, F32> &&
                          is_same_v<ScaleDataType, F32> && is_same_v<DscaleDbiasDataType, F32> &&
                          is_same_v<MeanVarDataType, F32>)
        {
            if constexpr(Rank == 4 && NumReduceDim == 3 && is_same_v<DyElementwiseOp, PassThrough>)
            {
                add_device_batchnorm_backward_rank_4_3_bf16_scale_f32_instances(op_ptrs);
            }
        }
