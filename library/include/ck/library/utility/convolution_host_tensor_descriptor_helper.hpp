...
std::vector<std::size_t> get_layout_transpose_gnchw_to_old()
{
    ...
    if constexpr(std::is_base_of<ck::tensor_layout::convolution::NGCHWcBase, OldLayout>::value)
    {
        return {1, 0, 2, 3, 4, 5};
    }
    if constexpr(std::is_base_of<ck::tensor_layout::convolution::GKCYXcBase, OldLayout>::value)
    {
        return {0, 1, 2, 3, 4, 5};
    }
                      ...
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::GNKHW>)
    {
        return {0, 1, 2, 3, 4};
                      ...
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::GKCYXc32>)
    {
        return {0, 1, 2, 3, 4, 5};
    }
                      ...
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::NDHWGK>)
    {
        return {4, 0, 5, 1, 2, 3};
                      ...
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::KGCX> ||
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::NGKW>)
    {
        return {1, 0, 2, 3};
                      ...
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::KGCYX> ||
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::NGKHW>)
    {
        return {1, 0, 2, 3, 4};
                      ...
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::KGCZYX> ||
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::NGKDHW> ||
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::NGCHWc32> ||
                      ck::is_same_v<OldLayout, ck::tensor_layout::convolution::NGKHWk32>)
    {
        return {1, 0, 2, 3, 4, 5};
    }
    else
...
make_input_host_tensor_descriptor_g_n_c_wis_packed(const ck::utils::conv::ConvParam& param)
{
    ...
    // add NGCHWcBase
    else if constexpr(std::is_base_of<ck::tensor_layout::convolution::NGCHWcBase, InLayout>::value)
    {
        const auto C_VECT = InLayout::x;
        const auto C_PAD  = ((param.C_ + C_VECT - 1) / C_VECT) * C_VECT;

        physical_lengths = std::vector<std::size_t>{static_cast<std::size_t>(param.N_),
                                                    static_cast<std::size_t>(param.G_),
                                                    static_cast<std::size_t>(C_PAD / C_VECT)};
        physical_lengths.insert(physical_lengths.end(),
                                param.input_spatial_lengths_.begin(),
                                param.input_spatial_lengths_.begin() + param.num_dim_spatial_);
        physical_lengths.push_back(static_cast<std::size_t>(C_VECT));
    }
    else if constexpr(ck::is_same_v<InLayout, ck::tensor_layout::convolution::NGCHWc32>)
    {
        const auto C_VECT = 32;
        const auto C_PAD  = ((param.C_ + C_VECT - 1) / C_VECT) * C_VECT;
...
make_weight_host_tensor_descriptor_g_k_c_xs_packed(const ck::utils::conv::ConvParam& param)
{
    ...
    // add GKCYXcBase
    else if constexpr(std::is_base_of<ck::tensor_layout::convolution::GKCYXcBase, WeiLayout>::value)
    {

        const auto C_VECT = WeiLayout::x;
        const auto C_PAD  = ((param.C_ + C_VECT - 1) / C_VECT) * C_VECT;
    ...
    else if constexpr(ck::is_same_v<WeiLayout, ck::tensor_layout::convolution::GKCYXc32>)
    {
...
make_output_host_tensor_descriptor_g_n_k_wos_packed(const ck::utils::conv::ConvParam& param)
{
    ...
    // add NGCHWcBase
    else if constexpr(std::is_base_of<ck::tensor_layout::convolution::NGCHWcBase, OutLayout>::value)
    {
        const auto K_VECT = OutLayout::x;
        const auto K_PAD  = ((param.K_ + K_VECT - 1) / K_VECT) * K_VECT;

        physical_lengths = std::vector<std::size_t>{static_cast<std::size_t>(param.N_),
                                                    static_cast<std::size_t>(param.G_),
                                                    static_cast<std::size_t>(K_PAD / K_VECT)};

        physical_lengths.insert(physical_lengths.end(),
                                param.output_spatial_lengths_.begin(),
                                param.output_spatial_lengths_.begin() + param.num_dim_spatial_);

        physical_lengths.push_back(K_VECT);
    }
    else if constexpr(ck::is_same_v<OutLayout, ck::tensor_layout::convolution::NGKHWk32>)
    {
        const auto K_VECT = 32;
        const auto K_PAD  = ((param.K_ + K_VECT - 1) / K_VECT) * K_VECT;
