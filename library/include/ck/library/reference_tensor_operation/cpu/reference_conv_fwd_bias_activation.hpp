...
//
// @brief      Reference implementation for forward convolution bias activation.
//
// @paragraph
...
struct ReferenceConvFwdBiasActivation : public device::BaseOperator
{
    // Argument
    struct Argument : public device::BaseArgument
    {
        ...
        const Tensor<OutDataType>& bias_;
    ...
    {
        using Argument = ReferenceConvFwdBiasActivation::Argument;
        ...
        float Run(const Argument& arg)
        {
                ...
                if constexpr(NDimSpatial == 1)
                {
                    auto func = [&](auto g, auto n, auto k, auto wo) {
                        ...
                        for(std::size_t c = 0; c < arg.weight_.GetLengths()[2]; ++c)
                        {
                            ...
                            {
                                auto wi =
                                    static_cast<ck::long_index_t>(wo * arg.conv_strides_[0]) +
                        ...
                        arg.out_element_op_(v_out, v_acc, ck::type_convert<float>(arg.bias_(g, k)));
                        ...
                        for(std::size_t c = 0; c < arg.weight_.GetLengths()[2]; ++c)
                        {
                            for(std::size_t z = 0; z < arg.weight_.GetLengths()[3]; ++z)
                            {
                                ...
                                for(std::size_t y = 0; y < arg.weight_.GetLengths()[4]; ++y)
                                {
                                    ...
                                    {
                                        auto wi =
                                            static_cast<ck::long_index_t>(wo *
                                                                          arg.conv_strides_[2]) +
                                            static_cast<ck::long_index_t>(x *
                                               ...
                                               arg.input_.GetLengths()[5])
                                        {
                                            ...
                                            arg.in_element_op_(v_in,
                                                               ck::type_convert<float>(arg.input_(
                                                                   g, n, c, di, hi, wi)));
            ...
            {
                if((arg.input_.GetNumOfDimension() == NDimSpatial + 4 &&
                    arg.weight_.GetNumOfDimension() == NDimSpatial + 4 &&
                    arg.output_.GetNumOfDimension() == NDimSpatial + 4))
                {
                    if constexpr(NDimSpatial == 2)
                    {
                        auto func = [&](auto g, auto n, auto k, auto ho, auto wo, auto k_vect) {
                            float v_acc        = 0;
                            std::size_t K_VECT = arg.output_.GetLengths()[5];

                            for(std::size_t c = 0; c < arg.weight_.GetLengths()[2]; ++c)
                            {
                                for(std::size_t c_vect = 0; c_vect < arg.weight_.GetLengths()[5];
                                    ++c_vect)
                                {
                                    for(std::size_t y = 0; y < arg.weight_.GetLengths()[3]; ++y)
                                    {
                                        ...
                                        for(std::size_t x = 0; x < arg.weight_.GetLengths()[4]; ++x)
                                        {
                                            auto wi =
                                                static_cast<ck::long_index_t>(
                                                    wo * arg.conv_strides_[1]) +
                                                static_cast<ck::long_index_t>(
                                                    x * arg.conv_dilations_[1]) -
                                                static_cast<ck::long_index_t>(arg.in_left_pads_[1]);
                                                   ...
                                                   arg.input_.GetLengths()[4])
                                            {
                                                ...
                                                arg.in_element_op_(
                                                    v_in,
                                                    ck::type_convert<float>(
                                                        arg.input_(g, n, c, hi, wi, c_vect)));

                                                arg.wei_element_op_(
                                                    v_wei,
                                                    ck::type_convert<float>(arg.weight_(
                            ...
                            arg.out_element_op_(
                                v_out,
                                v_acc,
                                ck::type_convert<float>(arg.bias_(g, k * K_VECT + k_vect)));

                            arg.output_(g, n, k, ho, wo, k_vect) =
                    ...
                    arg.output_.GetNumOfDimension() == NDimSpatial + 3))
                {
                    if constexpr(NDimSpatial == 2)
                    {
                        auto func = [&](auto g, auto n, auto k, auto ho, auto wo) {
                            ...
                            arg.out_element_op_(
                                v_out, v_acc, ck::type_convert<float>(arg.bias_(g, k)));
    ...
    std::string GetTypeString() const override
    {
        ...
        // clang-format off
        str << "ReferenceConvFwdBiasActivation"
            << std::endl;
        // clang-format on
