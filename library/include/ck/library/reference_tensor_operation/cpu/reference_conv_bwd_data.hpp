        ...
        float Run(const Argument& arg)
        {
                            ...
                            if(w_tmp % arg.conv_strides_[0] == 0)
                            {
                                ...
                                if(wo >= 0 && ck::type_convert<std::size_t>(wo) < Wo)
                                {
                                    for(std::size_t k = 0; k < K; ++k)
                                    {
                                        ...
                                        arg.wei_element_op_(
                                            v_wei,
                                            ck::type_convert<float>(arg.weight_(g, k, c, x)));
            ...
            else
            {
                    ...
                    arg.output_.GetNumOfDimension() == NDimSpatial + 4))
                {
                    ...
                    {
                        auto f_nchw = [&](auto g, auto n, auto c, auto hi, auto wi, auto c_vect) {
                            std::size_t K = arg.weight_.GetLengths()[1];
                            std::size_t Y = arg.weight_.GetLengths()[3];
                            std::size_t X = arg.weight_.GetLengths()[4];
                ...
                else if((arg.input_.GetNumOfDimension() == NDimSpatial + 3 &&
                         arg.weight_.GetNumOfDimension() == NDimSpatial + 4 &&
                         arg.output_.GetNumOfDimension() == NDimSpatial + 4))
                {
                    if constexpr(NDimSpatial == 2)
                    {
                        auto f_nchw = [&](auto g, auto n, auto c, auto hi, auto wi) {
                            ...
                            std::size_t Y      = arg.weight_.GetLengths()[3];
                            std::size_t X      = arg.weight_.GetLengths()[4];
                            std::size_t C_VECT = arg.weight_.GetLengths()[5];
